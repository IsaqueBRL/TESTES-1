    ...
    ['chart-data-inicio', 'chart-data-fim', 'sales-date', 'sales-due-date', 'vendidos-data-inicio', 'vendidos-data-fim',
     ...
     'compra-data-vencimento', 'pedido-compra-previsao-entrega', 'dar-baixa-compra-data',
    ...
    document.querySelectorAll('.compras-subview-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            document.querySelectorAll('.compras-subview-btn').forEach(b => b.classList.remove('ativo'));
            btn.classList.add('ativo');
            const view = btn.getAttribute('data-compras-view');
    ...
    function criarLinhaItemPedidoCompra() {
        const div = document.createElement('div');
        div.className = 'linha-item-pedido-compra';
        div.style.cssText = 'display:flex; gap:8px; align-items:flex-end; margin-bottom:8px; flex-wrap:wrap; background:var(--bg-soft); padding:8px; border-radius: 3px;';

        const selectInsumo = document.createElement('select');
        selectInsumo.className = 'item-pedido-insumo';
        selectInsumo.style.cssText = 'flex:2 1 180px; margin:0;';
        selectInsumo.innerHTML = '<option value="">-- Selecione o Insumo --</option>' +
            Object.keys(allLoadedSuprimentos).map(key => `<option value="${key}">${allLoadedSuprimentos[key].name}</option>`).join('');

        const inputQtd = document.createElement('input');
        inputQtd.type = 'number'; inputQtd.min = '0.0001'; inputQtd.step = 'any'; inputQtd.placeholder = 'Qtd. Desejada';
        inputQtd.className = 'item-pedido-qtd';
        inputQtd.style.cssText = 'flex:1 1 100px; margin:0;';

        const inputPreco = document.createElement('input');
        inputPreco.type = 'number'; inputPreco.min = '0'; inputPreco.step = 'any'; inputPreco.placeholder = 'Preço Estimado (R$)';
        inputPreco.className = 'item-pedido-preco';
        inputPreco.style.cssText = 'flex:1 1 130px; margin:0;';

        const spanSubtotal = document.createElement('span');
        spanSubtotal.className = 'item-pedido-subtotal';
        spanSubtotal.style.cssText = 'flex:1 1 90px; font-weight:700; color:var(--text-strong); font-size:9.5pt; text-align:right; padding-top:4px;';
        spanSubtotal.innerText = 'R$ 0,00';
        ...
        btnRemover.className = 'btn-action-prod btn-delete-prod';
        btnRemover.style.cssText = 'flex:0 0 auto; padding:6px 10px;';
        btnRemover.addEventListener('click', () => { div.remove(); recalcularTotalPedidoCompra(); });

        selectInsumo.addEventListener('change', () => {
            const insumo = allLoadedSuprimentos[selectInsumo.value];
            inputPreco.value = insumo ? parseFloat(insumo.price || 0).toFixed(2) : '';
            recalcularSubtotalItemPedido();
        });

        function recalcularSubtotalItemPedido() {
            const qtd = parseFloat(inputQtd.value) || 0;
            const preco = parseFloat(inputPreco.value) || 0;
            spanSubtotal.innerText = `R$ ${formatMoeda(qtd * preco)}`;
            recalcularTotalPedidoCompra();
        }
        inputQtd.addEventListener('input', recalcularSubtotalItemPedido);
        inputPreco.addEventListener('input', recalcularSubtotalItemPedido);

        div.appendChild(selectInsumo);
        div.appendChild(inputQtd);
    ...
    function recalcularTotalPedidoCompra() {
        let total = 0;
        document.querySelectorAll('#pedido-compra-itens-lista .linha-item-pedido-compra').forEach(row => {
            const qtd = parseFloat(row.querySelector('.item-pedido-qtd').value) || 0;
            const preco = parseFloat(row.querySelector('.item-pedido-preco').value) || 0;
            total += qtd * preco;
        });
        document.getElementById('pedido-compra-valor-total-display').innerText = `R$ ${formatMoeda(total)}`;
        return parseFloat(total.toFixed(2));
    }
    ...
    // Mesmo lookup de fornecedor por CNPJ usado na Nova Compra, mas mais permissivo: aqui o CNPJ é
    // opcional (um pedido pode ser feito a um fornecedor ainda sem CNPJ cadastrado, só pelo nome).
    document.getElementById('pedido-compra-fornecedor-cnpj').addEventListener('blur', () => {
    ...
    document.getElementById('form-novo-pedido-compra').addEventListener('submit', (e) => {
        ...
        let itemInvalido = false;
        document.querySelectorAll('#pedido-compra-itens-lista .linha-item-pedido-compra').forEach(row => {
            const insumoKey = row.querySelector('.item-pedido-insumo').value;
            const qtdPedida = parseFloat(row.querySelector('.item-pedido-qtd').value);
            const precoEstimado = parseFloat(row.querySelector('.item-pedido-preco').value) || 0;
            if (!insumoKey || isNaN(qtdPedida) || qtdPedida <= 0) { itemInvalido = true; return; }
            const insumo = allLoadedSuprimentos[insumoKey];
            if (!insumo) { itemInvalido = true; return; }
            itens.push({ insumoKey, insumoNome: insumo.name, unidade: insumo.unit || '', qtdPedida, qtdRecebida: 0, precoEstimado });
        });
        if (itemInvalido || itens.length === 0) { showAlert('Preencha corretamente todos os itens do pedido (insumo e quantidade desejada).', 'warning'); return; }

        const prazoEntregaPrevisto = dataBRparaISO(document.getElementById('pedido-compra-previsao-entrega').value) || null;
        ...
        const novoPedido = {
            ...
            status: 'ENVIADO', dataPedido: hoje, prazoEntregaPrevisto, observacoes,
    ...
    function renderPedidosCompraTable() {
        ...
        let lista = Object.keys(allLoadedPedidosCompra)
            .map(key => {
                ...
                const valorEstimado = (p.itens || []).reduce((acc, it) => acc + (parseFloat(it.qtdPedida || 0) * parseFloat(it.precoEstimado || 0)), 0);
        ...
        if (lista.length === 0) {
            tbody.innerHTML = '<tr><td colspan="8" style="text-align:center; color:var(--text-faint); padding:20px;">Nenhum pedido de compra registrado ainda.</td></tr>';
            return;
        }
        ...
        lista.forEach(p => {
            ...
            const tr = document.createElement('tr');
            tr.innerHTML = `
                <td><strong style="color:#0854a0;">#${String(p.pedidoNumero).padStart(3,'0')}</strong></td>
                <td>${p.fornecedorNome}</td>
                <td>${p.dataPedido ? dataISOparaBR(p.dataPedido) : '-'}</td>
                <td>${p.prazoEntregaPrevisto ? dataISOparaBR(p.prazoEntregaPrevisto) : '<span style="color:var(--text-faint);">-</span>'}</td>
                ...
                <td style="white-space:nowrap;">
                    <button class="btn-action-prod btn-edit-prod" onclick="abrirDetalhePedidoCompra('${p.key}')">👁️ Ver</button>
                    ${podeReceberOuCancelar ? `<button class="btn-action-prod btn-edit-prod" style="background-color:var(--color-accent-orange);" onclick="receberNotaFiscalDePedido('${p.key}')">🧾 Receber Nota Fiscal</button>` : ''}
                    ${podeReceberOuCancelar ? `<button class="btn-action-prod btn-delete-prod" onclick="cancelarPedidoCompra('${p.key}')">✕ Cancelar</button>` : ''}
                    ${podeExcluir ? `<button class="btn-action-prod btn-delete-prod" onclick="excluirPedidoCompra('${p.key}')">🗑️ Excluir</button>` : ''}
                </td>
            `;
            tbody.appendChild(tr);
        });
    ...
    window.abrirDetalhePedidoCompra = function(key) {
        ...
        document.getElementById('detalhe-pedido-previsao').innerText = p.prazoEntregaPrevisto ? dataISOparaBR(p.prazoEntregaPrevisto) : '-';
    ...
    // "Receber Nota Fiscal": abre o modal de Nova Compra já pré-preenchido com o fornecedor e os itens
    // (na quantidade que ainda falta receber) do pedido - o usuário só confere/ajusta e confirma.
