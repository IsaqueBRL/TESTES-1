    ...
    function atualizarAlertasSistema() {
        ...
        const producoesPendentes = Object.values(allLoadedProductions).filter(o => o.status === 'CONFIRMADO');
        ...
        const badgeProducao = document.getElementById('badge-alerta-producao');
        ...
        if (badgeProducao) { badgeProducao.innerText = producoesPendentes.length; badgeProducao.style.display = producoesPendentes.length > 0 ? 'inline-flex' : 'none'; }

        const totalAlertas = produtosBaixoEstoque.length + insumosBaixoEstoque.length + vendasVencidas.length + producoesPendentes.length;
        ...
        producoesPendentes.forEach(o => {
            painel.innerHTML += `
                <div class="alerta-item" onclick="fecharPainelAlertasEIrPara('producao')">
                    <span class="alerta-icon"></span>
                    <div><strong>${o.name}</strong><br><span style="color:var(--color-accent-orange);">Pedido #${String(o.pedidoNumero).padStart(3,'0')} - ${o.yieldQty} un aguardando produção</span></div>
                </div>`;
        });
    }
    ...
    ['chart-data-inicio', 'chart-data-fim', 'sales-date', 'sales-due-date', 'vendidos-data-inicio', 'vendidos-data-fim',
     ...
     'sazonalidade-data-inicio', 'sazonalidade-data-fim'
    ...
    // Ao escolher um sabor numa OP NOVA (não numa OP já existente sendo editada), carrega a receita
    // padrão salva desse produto automaticamente — só cai no default genérico se não houver uma salva.
    document.getElementById('recipe-name').addEventListener('change', (e) => {
        if (currentProductionId !== null) return;
        const nomeEscolhido = e.target.value;
        const matchEntry = Object.entries(allLoadedProducts).find(([, v]) => v.name === nomeEscolhido);
        const yieldAtual = parseFloat(document.getElementById('yield').value) || undefined;
        const carregou = matchEntry ? carregarReceitaPadrao(matchEntry[0], yieldAtual) : false;
        if (!carregou) resetToDefaultIngredients();
    });
    ...
    document.getElementById('form-cadastro-insumo').addEventListener('submit', (e) => {
        ...
        set(newRef, { name, barcode, unit, quantity, price, estoqueMinimo })
        .then(() => {
            showAlert(`Matéria-Prima "${name}" cadastrada com sucesso!`, 'success');
    ...
    // ===== RECEITA PADRÃO POR PRODUTO (evita ter que redigitar a ficha técnica em toda OP nova) =====
    // Carrega a receita padrão salva de um produto nos campos da OP, escalando as quantidades de
    // insumo proporcionalmente caso a quantidade desejada seja diferente do rendimento base salvo.
    function carregarReceitaPadrao(productKey, yieldQtyAlvo) {
        const produto = allLoadedProducts[productKey];
        if (!produto || !produto.receitaPadrao || !Array.isArray(produto.receitaPadrao.ingredients)) return false;

        const baseYield = parseFloat(produto.receitaPadrao.yieldQty) || 1;
        const alvo = parseFloat(yieldQtyAlvo) || baseYield;
        const fator = alvo / baseYield;

        const container = document.getElementById('ingredients-container');
        container.innerHTML = '';
        produto.receitaPadrao.ingredients.forEach(ing => {
            container.appendChild(createIngredientRow({
                name: ing.name,
                price: ing.price,
                usedQty: parseFloat((parseFloat(ing.usedQty || 0) * fator).toFixed(4))
            }));
        });
        document.getElementById('yield').value = alvo;
        calculateCosts();
        return true;
    }

    document.getElementById('btn-salvar-receita-padrao').addEventListener('click', () => {
        const nomeSelecionado = document.getElementById('recipe-name').value;
        if (!nomeSelecionado) { showAlert('Selecione um produto cadastrado no catálogo antes de salvar a receita padrão.', 'warning'); return; }

        const matchEntry = Object.entries(allLoadedProducts).find(([, v]) => v.name.toLowerCase() === nomeSelecionado.toLowerCase());
        if (!matchEntry) { showAlert('Este produto não está mais no catálogo — não é possível salvar uma receita padrão para ele.', 'warning'); return; }

        const yieldQty = parseFloat(document.getElementById('yield').value) || 0;
        const linhas = Array.from(document.querySelectorAll('#ingredients-container .ingredient-row-wrapper'));
        if (yieldQty <= 0 || linhas.length === 0) { showAlert('Monte a ficha técnica (rendimento e ao menos 1 insumo) antes de salvar como padrão.', 'warning'); return; }

        const ingredients = linhas.map(row => ({
            name: row.querySelector('.ing-name').value,
            price: parseFloat(row.querySelector('.ing-price').value) || 0,
            usedQty: parseFloat(row.querySelector('.ing-used-qty').value) || 0
        }));

        update(ref(db, `produtos/${matchEntry[0]}`), { receitaPadrao: { yieldQty, ingredients } }).then(() => {
            showAlert(`Receita padrão de "${nomeSelecionado}" salva. Da próxima vez ela carrega sozinha.`, 'success');
        });
    });

    // ===== SOLICITAR PRODUÇÃO (FLUXO RÁPIDO) =====
    // Abre a OP já pré-preenchida com a receita padrão do sabor escolhido, escalada pra quantidade
    // desejada, com status CONFIRMADO (a mesma "fila de produção pendente" que já existe no sistema
    // — conta na Previsão do Catálogo e aparece na fila de pendentes da Aba 1). A pessoa que for
    // produzir ainda pode revisar/ajustar tudo antes de salvar.
    document.getElementById('btn-solicitar-producao').addEventListener('click', () => {
        const selectSabor = document.getElementById('solicitar-producao-sabor');
        selectSabor.innerHTML = '';
        const comReceita = Object.entries(allLoadedProducts).filter(([, p]) => p.receitaPadrao && Array.isArray(p.receitaPadrao.ingredients) && p.receitaPadrao.ingredients.length > 0);

        document.getElementById('solicitar-producao-sem-receitas').style.display = comReceita.length === 0 ? 'block' : 'none';
        document.getElementById('form-solicitar-producao').style.display = comReceita.length === 0 ? 'none' : 'block';
        ...
        popularTodosSelectsDeLocal();
        const selLocal = document.getElementById('solicitar-producao-local');
        selLocal.value = localPadraoKey || '';

        document.getElementById('solicitar-producao-quantidade').value = 10;
        document.getElementById('modal-solicitar-producao').style.display = 'flex';
    });
    document.getElementById('close-modal-solicitar-producao').addEventListener('click', () => {
        document.getElementById('modal-solicitar-producao').style.display = 'none';
    });

    document.getElementById('form-solicitar-producao').addEventListener('submit', (e) => {
        e.preventDefault();
        const productKey = document.getElementById('solicitar-producao-sabor').value;
        const yieldQtyAlvo = parseFloat(document.getElementById('solicitar-producao-quantidade').value) || 0;
        const localKey = document.getElementById('solicitar-producao-local').value;
        const produto = allLoadedProducts[productKey];
        if (!produto || yieldQtyAlvo <= 0 || !localKey) return;

        document.getElementById('modal-solicitar-producao').style.display = 'none';

        currentProductionId = null;
        document.getElementById('pedido-numero-display').innerText = `Pedido #${String(getNextPedidoNumero()).padStart(3, '0')}`;
        populateRecipeNameSelect(produto.name);
        popularTodosSelectsDeLocal();
        const selLocalProducao = document.getElementById('producao-local-destino');
        selLocalProducao.value = localKey;
        selLocalProducao.disabled = false;

        carregarReceitaPadrao(productKey, yieldQtyAlvo);
        updateStatusSelectorVisual('CONFIRMADO');
        showCalculosSection();
        showAlert(`Ficha técnica de "${produto.name}" carregada pra ${yieldQtyAlvo} unidades. Confira e clique em "Salvar" pra deixar o pedido na fila de produção.`, 'info');
    });
    ...
    function initDatabaseSync() {
        ...
        // Ordens confirmadas ainda não produzidas = fila de "pra fazer". É a mesma lista que já
        // conta pra Previsão do Catálogo, só que aqui em destaque, pronta pra alguém pegar e produzir.
        function renderFilaPendentesProducao() {
            const caixa = document.getElementById('fila-pendentes-producao');
            const lista = document.getElementById('fila-pendentes-producao-lista');
            const pendentes = Object.entries(allLoadedProductions)
                .filter(([, o]) => o.status === 'CONFIRMADO')
                .sort((a, b) => (a[1].timestamp || 0) - (b[1].timestamp || 0));

            if (pendentes.length === 0) { caixa.style.display = 'none'; lista.innerHTML = ''; return; }
            caixa.style.display = 'block';

            lista.innerHTML = pendentes.map(([key, o]) => {
                const minutos = Math.max(0, Math.floor((Date.now() - (o.timestamp || Date.now())) / 60000));
                let tempo;
                if (minutos < 60) tempo = `há ${minutos} min`;
                else if (minutos < 1440) tempo = `há ${Math.floor(minutos / 60)}h`;
                else tempo = `há ${Math.floor(minutos / 1440)} dia(s)`;
                return `
                    <div class="alerta-item" style="cursor:default;">
                        <span class="alerta-icon"></span>
                        <div style="flex:1;">
                            <strong>${o.name}</strong> — ${o.yieldQty} un
                            <br><span style="color:var(--text-muted); font-size:8.5pt;">Pedido #${String(o.pedidoNumero).padStart(3,'0')} · ${tempo}${o.localProducaoNome ? ' · ' + o.localProducaoNome : ''}</span>
                        </div>
                        <button type="button" class="btn-secondary" style="background-color: var(--color-accent-orange); padding:6px 14px; font-size:9pt;" onclick="openProductionOrder('${key}')">Produzir</button>
                    </div>`;
            }).join('');
        }
        ...
        function renderSuprimentosTable() {
            ...
            let listaCompleta = Object.keys(data).map(key => {
                ...
                return { key, name: data[key].name, barcode: data[key].barcode || '', price: parseFloat(data[key].price || 0), quantity, unit: data[key].unit || 'g', estoqueMinimo, baixoEstoque };
            });
            ...
            if (filtroBuscaSuprimentos.trim() !== '') {
                ...
                lista = lista.filter(item => normalizarBusca(item.name).includes(termo) || normalizarBusca(item.barcode).includes(termo));
            }
        ...
        (function initProdutosVendaTooltip() {
            ...
            function montarTabelaProdutos(saleKey) {
                ...
                const linhas = itens.map(i => `<tr><td>${i.productName || 'Produto'}</td><td class="tt-qtd">${i.quantity || 1}x</td></tr>`).join('');
    ...
    function renderComprasTable() {
        ...
        if (lista.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; color:var(--text-faint); padding:20px;">Nenhuma compra registrada ainda.</td></tr>';
            return;
        }
    ...
    function abrirDetalheCompra(key) {
        ...
        (compra.itens || []).forEach(it => {
            const tr = document.createElement('tr');
            const nomeExibido = it.semEstoque ? `${it.insumoNome || '-'} <span style="font-size:7.5pt; color:#085caf;">(sem estoque)</span>` : (it.insumoNome || '-');
    ...
    document.getElementById('form-nova-compra').addEventListener('submit', (e) => {
        ...
        // Monta a lista de contas que vão pagar essa compra
        const pagamentos = [];
        ...
        if (pagamentoInvalido || pagamentos.length === 0) { showAlert('Selecione ao menos uma conta válida para abater o valor da compra, com um valor informado.', 'warning'); return; }
        ...
        if (Math.abs(valorTotalCompraFinal - valorTotalPagamentos) > 0.01) {
        ...
        const executarCompra = () => {
            ...
            promessas.push(set(compraRef, {
                ...
                valorTotal: valorTotalCompraFinal, movimentacoesKeys, timestamp, ...dadosNota
            }));
            ...
            Promise.all(promessas).then(() => {
                ...
                document.getElementById('form-nova-compra').reset();
                showAlert(`Compra registrada! R$ ${formatMoeda(valorTotalCompraFinal)} abatidos das contas escolhidas e estoque dos insumos atualizado.`, 'success');
            });
        };
