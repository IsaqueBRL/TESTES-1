    ...
    ['chart-data-inicio', 'chart-data-fim', 'sales-date', 'sales-due-date', 'vendidos-data-inicio', 'vendidos-data-fim',
     ...
     'compra-data-vencimento', 'pedido-compra-data-lancamento', 'pedido-compra-data-vencimento', 'dar-baixa-compra-data',
    ...
    document.querySelectorAll('.pedido-prazo-rapido-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            const dias = parseInt(btn.getAttribute('data-dias'));
            const baseISO = dataBRparaISO(document.getElementById('pedido-compra-data-lancamento').value) || formatDateInputValue(new Date());
            const dataBase = new Date(baseISO + 'T12:00:00');
            dataBase.setDate(dataBase.getDate() + dias);
            document.getElementById('pedido-compra-data-vencimento').value = dataISOparaBR(formatDateInputValue(dataBase));
        });
    });
    ...
    document.getElementById('btn-trigger-modal-novo-pedido-compra').addEventListener('click', () => {
        ...
        document.getElementById('pedido-compra-numero-display').innerText = '#' + String(getNextNumeroPedidoCompra()).padStart(3, '0');
    ...
    document.getElementById('form-novo-pedido-compra').addEventListener('submit', (e) => {
        ...
        const novoPedido = {
            ...
            status: 'ENVIADO', dataPedido: dataLancamento, dataVencimento, observacoes,
