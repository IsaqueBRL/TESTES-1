import { initializeApp } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-app.js";
    ...
    // Guarda as chaves (key) dos insumos em que o usuário já clicou dentro do pop-up "Escolha a
    // Matéria-Prima para a OP" nesta sessão de abertura do modal - usado só pra marcar a linha em
    // verde (feedback visual persistente), já que o modal agora fica aberto pra adicionar vários.
    const insumosJaAdicionadosNoPopUp = new Set();
        ...
        if(btnId) {
            document.getElementById(btnId).addEventListener('click', () => {
                if (modalId === 'modal-selecionar-insumo') {
                    ...
                    if (inputBusca) inputBusca.value = '';
                    if (btnLimpar) btnLimpar.style.display = 'none';
                    // Reinicia a marcação visual dos insumos já escolhidos toda vez que o pop-up é
                    // reaberto (ver insumosJaAdicionadosNoPopUp mais abaixo).
                    insumosJaAdicionadosNoPopUp.clear();
                    renderPopUpInsumos();
                }
    ...
    function renderPopUpInsumos() {
        ...
        suprimentosList.forEach(sup => {
            ...
            if (insumosJaAdicionadosNoPopUp.has(sup.key)) tr.classList.add('insumo-pop-row-selecionado');
            tr.innerHTML = `
            ...
            tr.addEventListener('click', () => {
                ...
                // sequência sem precisar reabrir a lista a cada um. A linha fica marcada em verde
                // (persistente, não só um flash) pra sempre dar pra ver de relance o que já foi escolhido.
                insumosJaAdicionadosNoPopUp.add(sup.key);
                tr.classList.add('insumo-pop-row-selecionado');
            });
