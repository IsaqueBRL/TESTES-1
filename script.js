import { initializeApp } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-app.js";
    ...
    // Guarda a key do Pedido de Compra atualmente aberto no modal de detalhe, pra poder recarregar o
    // modal automaticamente se os dados dele mudarem enquanto está aberto (ex.: acabou de salvar um
    // Recebimento ou uma Nota Fiscal vinculada a ele).
    let pedidoDetalheAbertoKey = null;
    ...
    let selecionadosCatalogo = new Set();
    ...
    // CORREÇÃO DE BUG: antes a busca da OP era feita comparando pelo NOME do produto (o.name), então
    // renomear um produto "perdia" o histórico de custo das OPs antigas, e dois produtos com nomes
    // iguais/parecidos podiam ter os custos misturados. Agora prioriza o vínculo pela chave única do
    // produto (o.productKey === prodKey, gravada em cada OP a partir desta correção - ver
    // saveProductionOrder), e só cai para o nome em OPs antigas que ainda não têm essa chave (dados
    // legados). Também passou a ORDENAR por timestamp antes de pegar a última, igual já era feito na
    // tela "Evolução do Custo por Unidade" - antes pegava só a última posição do array (Object.values),
    // que não é necessariamente a mais recente se a ordem de carregamento não for cronológica.
    function getCustoEfetivoProduto(prod, prodKey) {
        if (prod && prod.costPrice !== undefined && prod.costPrice !== null && prod.costPrice !== '') {
            return parseFloat(prod.costPrice) || 0;
        }
        if (!prod) return 0;
        let opsDoProduto = prodKey ? Object.values(allLoadedProductions).filter(o => o.productKey === prodKey) : [];
        if (opsDoProduto.length === 0) {
            opsDoProduto = Object.values(allLoadedProductions).filter(o => !o.productKey && o.name.toLowerCase() === (prod.name || '').toLowerCase());
        }
        if (opsDoProduto.length === 0) return 0;
        opsDoProduto.sort((a, b) => (a.timestamp || 0) - (b.timestamp || 0));
        return opsDoProduto[opsDoProduto.length - 1].costPerUnit || 0;
    }

    // AGRUPAMENTO DE VENDAS POR CLIENTE NOS RELATÓRIOS (Contas a Receber, Ranking, Rentabilidade,
    // Descontos): usa a chave única do cadastro (clientKey, gravada na venda a partir desta correção)
    // quando existir - assim variação de maiúscula/espaço no nome não vira cliente duplicado, e uma
    // renomeação posterior no cadastro (Aba 6) não deixa o histórico "órfão" sob o nome antigo. Só cai
    // para o nome normalizado (trim + minúsculas) em vendas antigas, salvas antes desta correção, que
    // ainda não têm clientKey gravado.
    function getClienteAgrupamento(s) {
        if (s.clientKey && allLoadedClientes[s.clientKey]) {
            return { chave: 'k:' + s.clientKey, nome: allLoadedClientes[s.clientKey].name || s.clientName || 'Cliente não informado' };
        }
        const nome = (s.clientName || 'Cliente não informado').trim();
        return { chave: 'n:' + nome.toLowerCase(), nome };
    }

    // BUSCA O ESTOQUE, UM E CÓDIGO INTERNO ATUAIS DE UM INSUMO DIRETO DA ABA 3 (SUPRIMENTOS)
    function getEstoqueEUmAtual(name) {
        ...
        return {
            ...
            um: match ? (match.unit || '-') : '-',
            codigoInterno: match ? String(parseInt(match.codigoInterno) || 0).padStart(3, '0') : '-'
        };
    }
    ...
    ['chart-data-inicio', 'chart-data-fim', 'sales-date', 'sales-due-date', 'vendidos-data-inicio', 'vendidos-data-fim',
     ...
     'compra-data-vencimento', 'pedido-compra-data-lancamento', 'pedido-compra-data-faturamento', 'dar-baixa-compra-data',
     'sazonalidade-data-inicio', 'sazonalidade-data-fim', 'orcamento-data-criacao', 'orcamento-prazo', 'recebimento-data'
    ...
    let pendingPagamento = null; // { valorPago, creditoUsado, creditoGerado, clienteKey, pagamentos: [{contaKey, contaNome, valor}] } definido só após confirmar o pop-up de pagamento
    ...
    function atualizarResumoPagamentoNaVenda() {
        ...
        const rowContas = document.getElementById('pdv-row-contas-pagamento');
        ...
        // Lista cada forma de pagamento separada pela conta em que o cliente pagou (ex: parte em
        // dinheiro, parte em depósito em outra conta) - a mesma venda pode ter várias linhas aqui.
        const pagamentos = pendingPagamento.pagamentos || [];
        if (pagamentos.length > 0) {
            rowContas.style.display = 'block';
            document.getElementById('pdv-summary-contas-pagamento-lista').innerHTML = pagamentos.map(p => `
                <div style="display:flex; justify-content:space-between; padding:2px 0;">
                    <span>${p.contaNome}</span>
                    <strong>R$ ${formatMoeda(p.valor)}</strong>
                </div>
            `).join('');
        } else {
            rowContas.style.display = 'none';
    ...
    function abrirModalPagamento() {
        ...
        // Recria as linhas de forma de pagamento a partir do que já foi confirmado antes (se estiver
        // reabrindo o pop-up pra ajustar) ou começa com uma única linha já preenchida com o total da
        // venda - permite dividir o pagamento entre várias contas (ex: parte em dinheiro, parte em depósito).
        const formasContainer = document.getElementById('pgto-formas-container');
        formasContainer.innerHTML = '';
        if (pendingPagamento && pendingPagamento.pagamentos && pendingPagamento.pagamentos.length > 0) {
            pendingPagamento.pagamentos.forEach(p => criarLinhaFormaPagamento(p.contaKey, p.valor));
        } else {
            criarLinhaFormaPagamento('', totalLiquido);
        }
    ...
    // Cria uma linha de "forma de pagamento" (conta + valor) dentro do pop-up de confirmar pagamento,
    // permitindo dividir o valor recebido entre dinheiro físico, depósito, pix etc.
    function criarLinhaFormaPagamento(contaKey = '', valor = '') {
        const row = document.createElement('div');
        row.className = 'pgto-forma-row';
        const opcoesContas = ['<option value="">-- Selecione a conta --</option>']
            .concat(Object.keys(allLoadedContas).map(key => `<option value="${key}">${allLoadedContas[key].name}</option>`))
            .join('');
        row.innerHTML = `
            <select class="pgto-forma-conta">${opcoesContas}</select>
            <div class="pgto-forma-row-bottom">
                <div>
                    <label>Valor (R$)</label>
                    <input type="number" class="pgto-forma-valor" min="0" step="0.01" placeholder="Ex: 12.00">
                </div>
                <button type="button" class="btn-remove-forma-pagamento" title="Remover esta forma de pagamento">✕</button>
            </div>
        `;
        row.querySelector('.pgto-forma-conta').value = contaKey || '';
        row.querySelector('.pgto-forma-valor').value = valor !== '' && valor !== null && valor !== undefined ? valor : '';
        row.querySelector('.pgto-forma-valor').addEventListener('input', atualizarResumoPagamentoModal);
        row.querySelector('.pgto-forma-conta').addEventListener('change', atualizarResumoPagamentoModal);
        row.querySelector('.btn-remove-forma-pagamento').addEventListener('click', () => {
            row.remove();
            atualizarResumoPagamentoModal();
        });
        document.getElementById('pgto-formas-container').appendChild(row);
        return row;
    }
    document.getElementById('btn-add-forma-pagamento').addEventListener('click', (e) => { e.preventDefault(); criarLinhaFormaPagamento(); });

    // Soma o valor de todas as linhas de forma de pagamento já adicionadas no pop-up.
    function somarFormasPagamentoModal() {
        let soma = 0;
        document.querySelectorAll('#pgto-formas-container .pgto-forma-valor').forEach(inp => { soma += parseFloat(inp.value) || 0; });
        return soma;
    }

    // Toda linha que já tem um valor preenchido mas ainda está sem conta escolhida pisca em vermelho -
    // é fácil o usuário completar o total da venda e esquecer de escolher pra onde foi o dinheiro.
    function destacarFormasSemConta() {
        let faltaAlguma = false;
        document.querySelectorAll('#pgto-formas-container .pgto-forma-row').forEach(row => {
            const valorLinha = parseFloat(row.querySelector('.pgto-forma-valor').value) || 0;
            const contaSelect = row.querySelector('.pgto-forma-conta');
            const semConta = valorLinha > 0 && !contaSelect.value;
            contaSelect.classList.toggle('pgto-forma-conta-alerta', semConta);
            if (semConta) faltaAlguma = true;
        });
        return faltaAlguma;
    }
    ...
    function atualizarResumoPagamentoModal() {
        ...
        const valorPago = somarFormasPagamentoModal();
        ...
        const faltaConta = destacarFormasSemConta();
        ...
        document.getElementById('btn-confirmar-pagamento').disabled = (total > 0 && falta > 0.001) || faltaConta;
    }
    ...
    document.getElementById('btn-confirmar-pagamento').addEventListener('click', () => {
        ...
        // Coleta todas as linhas de forma de pagamento preenchidas (dinheiro, depósito, etc.),
        // ignorando linhas deixadas em branco/zeradas, e exige conta selecionada em toda linha com valor.
        const pagamentos = [];
        let faltaConta = false;
        document.querySelectorAll('#pgto-formas-container .pgto-forma-row').forEach(row => {
            const valorLinha = parseFloat(row.querySelector('.pgto-forma-valor').value) || 0;
            const contaKeyLinha = row.querySelector('.pgto-forma-conta').value || null;
            if (valorLinha <= 0) return;
            if (!contaKeyLinha || !allLoadedContas[contaKeyLinha]) { faltaConta = true; return; }
            pagamentos.push({ contaKey: contaKeyLinha, contaNome: allLoadedContas[contaKeyLinha].name, valor: parseFloat(valorLinha.toFixed(2)) });
        });

        if (faltaConta) {
            if (Object.keys(allLoadedContas).length === 0) {
                showAlert('Não há nenhuma conta cadastrada. Cadastre uma conta na Aba 7 - Caixa antes de confirmar o pagamento.', 'warning');
            } else {
                showAlert('Selecione em qual conta cada forma de pagamento foi recebida.', 'warning');
            }
            return;
        }

        const valorPago = pagamentos.reduce((soma, p) => soma + p.valor, 0);
        const totalCoberto = creditoUsado + valorPago;

        if (totalCoberto < total - 0.001) {
            showAlert('O valor pago somado ao crédito usado ainda não cobre o total da venda.', 'warning');
            return;
        }
    ...
    function solicitarReversaoVenda(novoStatus) {
        ...
        const pagamentosRevertidos = (venda.pagamentos && venda.pagamentos.length > 0)
            ? venda.pagamentos
            : (venda.contaPagamentoNome && (venda.valorPago || 0) > 0 ? [{ contaNome: venda.contaPagamentoNome, valor: venda.valorPago }] : []);
        pagamentosRevertidos.forEach(p => { explicacao += `• O valor de R$ ${formatMoeda(p.valor)} lançado na conta "${p.contaNome}" (Aba 7 - Caixa) será estornado.\n`; });
    ...
    function reverterVendaPaga(novoStatus) {
        ...
        update(ref(db, `vendas/${currentSaleId}`), {
            ...
            pagamentos: null, movimentacoesCaixaKeys: null
        }).then(() => {
            ...
            // Desfaz TODAS as movimentações de Caixa geradas por esta venda (Aba 7) e devolve o saldo
            // de cada conta afetada ao valor de antes de a venda ter sido marcada como Paga - usa a
            // lista completa (venda.pagamentos/movimentacoesCaixaKeys) quando existir, com fallback
            // para os campos antigos de conta única em vendas salvas antes desse ajuste.
            const movKeys = venda.movimentacoesCaixaKeys && venda.movimentacoesCaixaKeys.length > 0
                ? venda.movimentacoesCaixaKeys
                : (venda.movimentacaoCaixaKey ? [venda.movimentacaoCaixaKey] : []);
            const pagamentosParaEstornar = venda.pagamentos && venda.pagamentos.length > 0
                ? venda.pagamentos
                : (venda.contaPagamentoKey && (venda.valorPago || 0) > 0 ? [{ contaKey: venda.contaPagamentoKey, valor: venda.valorPago }] : []);

            if (movKeys.length === 0 && pagamentosParaEstornar.length === 0) { finalizarReversao(); return; }

            const deltaPorConta = {};
            pagamentosParaEstornar.forEach(p => { if (p.contaKey) deltaPorConta[p.contaKey] = (deltaPorConta[p.contaKey] || 0) + (p.valor || 0); });

            const promessas = movKeys.map(key => remove(ref(db, `movimentacoesCaixa/${key}`)));
            Object.keys(deltaPorConta).forEach(contaKey => {
                if (!allLoadedContas[contaKey]) return;
                const saldoAtual = parseFloat(allLoadedContas[contaKey].saldo || 0);
                const novoSaldo = parseFloat((saldoAtual - deltaPorConta[contaKey]).toFixed(2));
                promessas.push(update(ref(db, `contas/${contaKey}`), { saldo: novoSaldo }));
            });
            Promise.all(promessas).then(finalizarReversao);
        });
    }
    ...
    setupModalEvents(null, 'modal-precos-massa-catalogo', 'close-modal-precos-massa-catalogo');
    ...
    function renderEstoqueTab() {
        ...
        Object.entries(allLoadedProducts).forEach(([key, p]) => {
            ...
            const custo = getCustoEfetivoProduto(p, key);
    ...
    window.openModalEditarProduto = function(key) {
        ...
        document.getElementById('edit-produto-custo').value = parseFloat(getCustoEfetivoProduto(prod, key)).toFixed(2);
    ...
    // ===== SELEÇÃO EM MASSA E EDIÇÃO DE PREÇOS EM MASSA NO CATÁLOGO (ABA 2) =====
    function atualizarBotaoPrecosMassaCatalogo() {
        const btn = document.getElementById('btn-editar-precos-massa-catalogo');
        document.getElementById('contador-selecionados-catalogo').innerText = selecionadosCatalogo.size;
        btn.style.display = selecionadosCatalogo.size > 0 ? 'inline-block' : 'none';

        const chkSelectAll = document.getElementById('chk-catalogo-select-all');
        const chksVisiveis = Array.from(document.querySelectorAll('#catalog-table-body .chk-catalogo-produto'));
        chkSelectAll.checked = chksVisiveis.length > 0 && chksVisiveis.every(c => c.checked);
        chkSelectAll.indeterminate = chksVisiveis.some(c => c.checked) && !chkSelectAll.checked;
    }

    document.getElementById('chk-catalogo-select-all').addEventListener('change', (e) => {
        document.querySelectorAll('#catalog-table-body .chk-catalogo-produto').forEach(chk => {
            chk.checked = e.target.checked;
            if (e.target.checked) selecionadosCatalogo.add(chk.dataset.key);
            else selecionadosCatalogo.delete(chk.dataset.key);
        });
        atualizarBotaoPrecosMassaCatalogo();
    });

    document.getElementById('btn-editar-precos-massa-catalogo').addEventListener('click', () => {
        if (selecionadosCatalogo.size === 0) return;
        document.getElementById('precos-massa-catalogo-contador').innerText = `${selecionadosCatalogo.size} produto(s) selecionado(s).`;
        document.getElementById('precos-massa-custo').value = '';
        document.getElementById('precos-massa-venda').value = '';
        document.getElementById('modal-precos-massa-catalogo').style.display = 'flex';
    });

    document.getElementById('form-precos-massa-catalogo').addEventListener('submit', (e) => {
        e.preventDefault();
        const custoTexto = document.getElementById('precos-massa-custo').value;
        const vendaTexto = document.getElementById('precos-massa-venda').value;

        // Campo vazio = não mexe naquele preço; só grava o que o usuário efetivamente preencheu.
        if (custoTexto.trim() === '' && vendaTexto.trim() === '') {
            document.getElementById('modal-precos-massa-catalogo').style.display = 'none';
            return;
        }

        const promessas = [];
        selecionadosCatalogo.forEach(key => {
            if (!allLoadedProducts[key]) return;
            if (custoTexto.trim() !== '') promessas.push(set(ref(db, `produtos/${key}/costPrice`), parseFloat(custoTexto) || 0));
            if (vendaTexto.trim() !== '') promessas.push(set(ref(db, `produtos/${key}/sellingPrice`), parseFloat(vendaTexto) || 0));
        });

        Promise.all(promessas).then(() => {
            document.getElementById('modal-precos-massa-catalogo').style.display = 'none';
            selecionadosCatalogo.clear();
            atualizarBotaoPrecosMassaCatalogo();
    ...
    function renderPopUpInsumos() {
        ...
        let suprimentosList = Object.keys(allLoadedSuprimentos).map(key => ({ key, ...allLoadedSuprimentos[key] }));

        if(suprimentosList.length === 0) {
            container.innerHTML = `<tr><td colspan="7" style="padding: 20px; text-align: center; color:#6a6d70; font-family: inherit;">Nenhuma matéria-prima localizada no sistema.</td></tr>`;
            return;
        }

        const termo = normalizarBusca(filtroBuscaPopUpInsumo);
        if(termo) {
            suprimentosList = suprimentosList.filter(sup => normalizarBusca(sup.name).includes(termo) || normalizarBusca(sup.barcode).includes(termo) || String(parseInt(sup.codigoInterno) || 0).padStart(3, '0').includes(filtroBuscaPopUpInsumo.trim()));
        }

        if(suprimentosList.length === 0) {
            container.innerHTML = `<tr><td colspan="7" style="padding: 20px; text-align: center; color:#6a6d70; font-family: inherit;">Nenhuma matéria-prima encontrada para "${filtroBuscaPopUpInsumo}".</td></tr>`;
            return;
        }

        // Nomes dos insumos que já estão na "estrutura de consumo" da OP agora mesmo (fonte real da
        // verdade) - é isso que decide quais linhas ficam marcadas em verde, então a marcação
        // permanece mesmo fechando e reabrindo o pop-up, e só some quando o insumo for removido de
        // fato (seja pelo ✕ aqui na lista, seja pelo ✕ da própria linha na estrutura de consumo).
        const nomesNaEstrutura = new Set(
            Array.from(document.querySelectorAll('#ingredients-container .ing-name')).map(inp => inp.value)
        );

        suprimentosList.forEach(sup => {
            const tr = document.createElement('tr');
            tr.className = 'insumo-pop-row';
            const jaEscolhido = nomesNaEstrutura.has(sup.name);
            if (jaEscolhido) tr.classList.add('insumo-pop-row-selecionado');
            tr.innerHTML = `
                <td><strong style="color:#0854a0;">${String(parseInt(sup.codigoInterno) || 0).padStart(3,'0')}</strong></td>
                ...
                <td style="text-align:center;">${jaEscolhido ? '<button type="button" class="btn-remove-insumo-pop-up" title="Remover este insumo da estrutura de consumo">✕</button>' : ''}</td>
            `;
            tr.addEventListener('click', () => {
                ...
                calculateCosts();
                atualizarEstadoVazioIngredientes();
                // Não fecha mais o modal ao clicar - permite ir adicionando vários insumos em
                // sequência sem precisar reabrir a lista a cada um.
                renderPopUpInsumos();
            });
            const btnRemover = tr.querySelector('.btn-remove-insumo-pop-up');
            if (btnRemover) {
                btnRemover.addEventListener('click', (e) => {
                    // Impede que o clique "vaze" pro <tr> (que adicionaria o insumo de novo) - o ✕
                    // remove TODAS as linhas desse insumo na estrutura de consumo, caso ele tenha
                    // sido adicionado mais de uma vez sem querer.
                    e.stopPropagation();
                    document.querySelectorAll('#ingredients-container .ingredient-row-wrapper').forEach(wrapper => {
                        if (wrapper.querySelector('.ing-name').value === sup.name) wrapper.remove();
                    });
                    calculateCosts();
                    atualizarEstadoVazioIngredientes();
                    renderPopUpInsumos();
    ...
    function createIngredientRow(data = { name: '', price: '', usedQty: '' }) {
        ...
        div.innerHTML = `
            <div><span class="badge-stock ing-codigo-display">${infoAtual.codigoInterno}</span></div>
            ...
            <div><input type="number" class="ing-price" value="${data.price}" step="0.01" required readonly style="background-color: #f5f6f7; cursor: not-allowed;"></div>
    ...
    // MOSTRA/ESCONDE O AVISO PEDINDO PARA VINCULAR INSUMOS QUANDO A ESTRUTURA DE CONSUMO ESTÁ VAZIA
    // (novas Ordens de Produção sempre começam sem nenhum insumo pré-selecionado)
    function atualizarEstadoVazioIngredientes() {
        const container = document.getElementById('ingredients-container');
        const vazio = document.getElementById('ingredients-empty');
        if (vazio) vazio.style.display = container.children.length === 0 ? 'block' : 'none';
    }

    // ATUALIZA OS CAMPOS DE ESTOQUE/UM/CÓDIGO INTERNO (SOMENTE LEITURA) DE TODAS AS LINHAS QUANDO A ABA 3 SINCRONIZAR
    function refreshIngredientStockDisplays() {
        document.querySelectorAll('#ingredients-container .ingredient-row-wrapper').forEach(wrapper => {
            ...
            row.querySelector('.ing-codigo-display').innerText = info.codigoInterno;
    ...
    function calculateCosts() {
        ...
        document.getElementById('res-selling-price').innerText = `R$ ${formatMoeda(sellingPrice)}`;
        document.getElementById('res-markup').innerText = `${formatMoeda(markup)}x`;
    ...
    function saveProductionOrder() {
        ...
        if (currentProductionId) {
            ...
            // Se esta OP (sendo editada) já tem a chave do produto gravada, preserva - assim, mesmo que o
            // produto seja renomeado depois, esta OP continua vinculada ao produto certo (correção do bug
            // "custo vinculado por nome"). Só cai para a busca por nome em OPs antigas sem essa chave.
            productKeyDaOp = allLoadedProductions[currentProductionId].productKey || null;
        }
        if (!productKeyDaOp) {
            const matchProdutoPeloNome = Object.entries(allLoadedProducts).find(([k, v]) => v.name.toLowerCase() === currentCalculatedData.name.toLowerCase());
            if (matchProdutoPeloNome) productKeyDaOp = matchProdutoPeloNome[0];
        }
        ...
        set(targetRef, {
            ...
            ingredients: currentCalculatedData.ingredients, timestamp: Date.now(), productKey: productKeyDaOp || null,
    ...
    // POPULA A BUSCA DE PRODUTOS NA ORDEM DE PRODUÇÃO (EVITA DUPLICIDADE DE CADASTRO)
    // O campo visível (#recipe-name-search) é um texto com autocomplete; o valor "de verdade"
    // usado pelo resto do sistema fica no input oculto #recipe-name, igual ao <select> de antes.
    let recipeNameOptionsCache = [];

    function populateRecipeNameSelect(forcedName) {
        const hiddenInput = document.getElementById('recipe-name');
        const searchInput = document.getElementById('recipe-name-search');
        const currentValue = forcedName !== undefined ? forcedName : hiddenInput.value;

        const nomes = Object.values(allLoadedProducts)
            .map(p => p.name)
            .sort((a, b) => a.localeCompare(b));

        recipeNameOptionsCache = nomes.map(name => ({ value: name, removido: false }));

        if(currentValue && !nomes.includes(currentValue)) {
            recipeNameOptionsCache.push({ value: currentValue, removido: true });
        }

        hiddenInput.value = currentValue || '';
        const optAtual = recipeNameOptionsCache.find(o => o.value === currentValue);
        searchInput.value = currentValue ? (optAtual && optAtual.removido ? `${currentValue} (produto removido do catálogo)` : currentValue) : '';
    }

    // Monta o dropdown de sugestões do campo de busca do produto na OP, destacando o trecho
    // digitado dentro do nome de cada produto que contém essas letras.
    function renderRecipeNameDropdown(query) {
        const dropdown = document.getElementById('recipe-name-dropdown');
        const q = (query || '').toLowerCase().trim();
        dropdown.innerHTML = '';

        const matches = recipeNameOptionsCache.filter(o => !q || o.value.toLowerCase().includes(q));

        matches.forEach(o => {
            const label = o.removido ? `${o.value} (produto removido do catálogo)` : o.value;
            const item = document.createElement('div');
            item.className = 'client-autocomplete-item';
            item.innerHTML = destacarTrechoBusca(label, q);
            item.addEventListener('mousedown', (e) => {
                e.preventDefault();
                document.getElementById('recipe-name').value = o.value;
                document.getElementById('recipe-name-search').value = label;
        ...
        if(matches.length === 0) {
            ...
            vazio.className = 'client-autocomplete-item';
            vazio.style.cssText = 'color: var(--text-faint); cursor: default;';
            vazio.innerText = 'Nenhum produto encontrado';
    ...
    // Deixa em negrito o trecho do texto que bate com o que o usuário digitou.
    function destacarTrechoBusca(texto, termo) {
    ...
    // LIMPA A ESTRUTURA DE CONSUMO E MOSTRA O AVISO PEDINDO PARA VINCULAR OS INSUMOS - toda nova
    // Ordem de Produção começa em branco, sem nenhum insumo pré-selecionado.
    function limparEstruturaDeConsumo() {
        const container = document.getElementById('ingredients-container');
        container.innerHTML = '';
        atualizarEstadoVazioIngredientes();
        calculateCosts();
    ...
    document.getElementById('btn-add-new-prazo').addEventListener('click', () => {
        const nomeInput = document.getElementById('new-prazo-nome');
        const diasInput = document.getElementById('new-prazo-dias');
        const nome = nomeInput.value.trim();
        // Aceita "7,14", "7/14" ou "7 14" como separador entre as parcelas
        const dias = diasInput.value.split(/[,\/\s]+/).map(d => parseInt(d.trim())).filter(d => !isNaN(d) && d >= 0);
        if (!nome) { showAlert('Informe um nome para o prazo (ex: "7/14", "À Vista", "30 dias").', 'warning'); return; }
        if (dias.length === 0) { showAlert('Informe pelo menos um dia válido (ex: "7,14" ou "30").', 'warning'); return; }
        push(ref(db, 'prazosPagamento'), { nome, dias, timestamp: Date.now() });
        nomeInput.value = ''; diasInput.value = '';
    });

    window.removePrazoPagamento = function(key) { remove(ref(db, `prazosPagamento/${key}`)); };

    document.getElementById('btn-gerenciar-prazos-pagamento').addEventListener('click', (e) => {
        e.preventDefault();
        const modalPrazos = document.getElementById('modal-gerenciar-prazos');
        modalPrazos.style.zIndex = '2000';
        modalPrazos.style.display = 'flex';
    });
    setupModalEvents(null, 'modal-gerenciar-prazos', 'close-modal-prazos');

    // Preenche o <select> de "Prazo de Pagamento" do Pedido de Compra com os prazos cadastrados -
    // chamada tanto ao carregar os dados do Firebase quanto ao reabrir o modal do pedido.
    function renderSelectPrazosPagamento(valorSelecionado) {
        const select = document.getElementById('pedido-compra-prazo-pagamento');
        const atual = valorSelecionado !== undefined ? valorSelecionado : select.value;
        select.innerHTML = '<option value="">-- Selecione --</option>';
        Object.keys(allLoadedPrazosPagamento).forEach(key => {
            const prazo = allLoadedPrazosPagamento[key];
            const diasArr = prazo.dias || [];
            const sufixo = (diasArr.length === 1 && diasArr[0] === 1) ? 'dia' : 'dias';
            const opt = document.createElement('option');
            opt.value = key;
            opt.innerText = `${prazo.nome} (${diasArr.join('/')} ${sufixo})`;
            select.appendChild(opt);
        });
        select.value = atual || '';
    }

    // Calcula a data de vencimento final de um prazo (a parcela mais distante) a partir da data de
    // lançamento do pedido - usada só pra manter o campo oculto de compatibilidade preenchido.
    function calcularVencimentoPorPrazo(dataLancamentoISO, dias) {
        if (!dias || dias.length === 0) return null;
        const maiorPrazo = Math.max(...dias);
        const base = new Date((dataLancamentoISO || formatDateInputValue(new Date())) + 'T12:00:00');
        base.setDate(base.getDate() + maiorPrazo);
        return formatDateInputValue(base);
    }

    // Sempre que o prazo escolhido (ou a data de lançamento) mudar, recalcula a data de vencimento
    // final e atualiza o campo oculto de compatibilidade com o resto do sistema.
    function atualizarVencimentoPeloPrazoSelecionado() {
        const prazoKey = document.getElementById('pedido-compra-prazo-pagamento').value;
        const prazo = allLoadedPrazosPagamento[prazoKey];
        const dataLancISO = dataBRparaISO(document.getElementById('pedido-compra-data-lancamento').value);
        const vencimentoISO = prazo ? calcularVencimentoPorPrazo(dataLancISO, prazo.dias) : null;
        document.getElementById('pedido-compra-data-vencimento').value = vencimentoISO ? dataISOparaBR(vencimentoISO) : '';
    }
    document.getElementById('pedido-compra-prazo-pagamento').addEventListener('change', atualizarVencimentoPeloPrazoSelecionado);
    document.getElementById('pedido-compra-data-lancamento').addEventListener('blur', atualizarVencimentoPeloPrazoSelecionado);
    ...
    document.getElementById('btn-execute-sale').addEventListener('click', () => {
      try {
        ...
        for(let row of rows) {
            ...
            const costBase = getCustoEfetivoProduto(prod, prodKey);
        ...
        let valorPago = 0, creditoUsado = 0, creditoGerado = 0, pagamentosValidos = [];
        if (selectedSalesStatus === 'PAGO') {
            if (!pendingPagamento) {
                showAlert('Clique em "Pago" para confirmar os valores do pagamento antes de salvar.', 'warning');
                restaurarEstoqueEmMemoria();
                return;
            }
            ...
            // Revalida cada forma de pagamento (a conta escolhida ainda precisa existir), já que o
            // pagamento pode ter sido dividido entre várias contas/dinheiro.
            if (valorPago > 0) {
                const formas = pendingPagamento.pagamentos || [];
                const algumaInvalida = formas.some(p => !p.contaKey || !allLoadedContas[p.contaKey]);
                if (formas.length === 0 || algumaInvalida) {
                    showAlert('Selecione novamente as contas de recebimento. Clique em "Pago" para reconfirmar.', 'warning');
                    restaurarEstoqueEmMemoria();
                    return;
                }
                pagamentosValidos = formas.map(p => ({ contaKey: p.contaKey, contaNome: allLoadedContas[p.contaKey].name, valor: p.valor }));
            }
        }
        ...
        // CORREÇÃO DE BUG: os relatórios por cliente (Contas a Receber, Ranking de Clientes, Rentabilidade
        // por Cliente, Descontos) agrupavam pelo TEXTO digitado em clientName - então "João Silva" e "joão
        // silva " (maiúscula/espaço diferente) viravam duas linhas, e renomear um cliente na Aba 6 deixava
        // o histórico anterior "órfão" sob o nome antigo. Agora toda venda com nome de cliente (exceto o
        // genérico "CONSUMIDOR FINAL") grava também a chave única do cadastro (clientKey) - vinculando a um
        // cliente já cadastrado quando o nome bater, ou criando o cadastro na hora se for a primeira vez
        // (antes só criava cadastro automático quando a venda gerava crédito). Isso dá aos relatórios um ID
        // estável pra agrupar, em vez do texto do nome.
        let clientKeyDaVenda = null;
        let novoClienteRefDaVenda = null;
        if (clientName && clientName.toUpperCase() !== 'CONSUMIDOR FINAL') {
            const nomeClienteNormalizado = clientName.toLowerCase();
            const clienteExistenteKey = Object.keys(allLoadedClientes).find(k => (allLoadedClientes[k].name || '').toLowerCase() === nomeClienteNormalizado);
            if (clienteExistenteKey) {
                clientKeyDaVenda = clienteExistenteKey;
            } else {
                novoClienteRefDaVenda = push(ref(db, 'clientes'));
                clientKeyDaVenda = novoClienteRefDaVenda.key;
            }
        }
        ...
        const payload = {
            clientName, clientKey: clientKeyDaVenda, salesDate, salesDueDate, status: selectedSalesStatus,
        ...
        if (selectedSalesStatus === 'PAGO') {
            ...
            if (pagamentosValidos.length > 0) {
                // Guarda a divisão completa do pagamento (dinheiro + depósito + etc.) em "pagamentos".
                // Quando é uma única forma, também preenche contaPagamentoKey/Nome no formato antigo,
                // pra manter compatibilidade com o app mobile e com o resto do código que ainda lê esses campos.
                payload.pagamentos = pagamentosValidos;
                if (pagamentosValidos.length === 1) {
                    payload.contaPagamentoKey = pagamentosValidos[0].contaKey;
                    payload.contaPagamentoNome = pagamentosValidos[0].contaNome;
                }
            }
        }
        ...
        // Se há dinheiro novo entrando, prepara uma movimentação de Caixa (Aba 7) por forma de
        // pagamento já com a chave gerada, para gravar as referências dentro da própria venda (permite
        // reverter depois). Uma linha "dinheiro" e outra "depósito", por exemplo, geram duas movimentações.
        const movimentacoesPreparadas = pagamentosValidos.map(p => ({ ...p, movRef: push(ref(db, 'movimentacoesCaixa')) }));
        if (movimentacoesPreparadas.length > 0) {
            payload.movimentacoesCaixaKeys = movimentacoesPreparadas.map(m => m.movRef.key);
            if (movimentacoesPreparadas.length === 1) payload.movimentacaoCaixaKey = movimentacoesPreparadas[0].movRef.key;
        }
        ...
        set(targetRef, payload).then(() => {
            ...
            const finalizarSalvamento = () => {
                ...
                pagamentosValidos.forEach(p => { msg += `\n\nR$ ${formatMoeda(p.valor)} foram lançados como entrada na conta "${p.contaNome}" (Aba 7 - Caixa).`; });
                showAlert(msg, 'success');
                currentSaleId = null;
            };

            // Lança uma entrada no Caixa para cada forma de pagamento e soma o valor pago ao saldo de
            // cada conta escolhida (somando por conta, caso a mesma conta apareça em mais de uma linha).
            const finalizarComCaixa = () => {
                if (movimentacoesPreparadas.length === 0) { finalizarSalvamento(); return; }
                const deltaPorConta = {};
                movimentacoesPreparadas.forEach(m => { deltaPorConta[m.contaKey] = (deltaPorConta[m.contaKey] || 0) + m.valor; });

                const promessas = movimentacoesPreparadas.map(m => set(m.movRef, {
                    contaKey: m.contaKey, contaNome: m.contaNome, tipo: 'entrada',
                    valor: m.valor, descricao: `Recebimento da venda de ${clientName}`,
                ...
                Promise.all(promessas).then(finalizarSalvamento).catch(err => {
                    // A venda já foi salva, mas o lançamento no Caixa falhou - avisa o usuário em vez
                    // de deixar o botão travado e o usuário sem saber o que aconteceu.
            ...
            // Cliente novo (nome que ainda não tinha cadastro): registra agora usando a chave já
            // preparada acima (clientKeyDaVenda), que é a mesma já salva na venda.
            if (novoClienteRefDaVenda) {
                set(novoClienteRefDaVenda, { name: clientName }).then(finalizarComCaixa);
            } else {
                finalizarComCaixa();
            }
        }).catch(err => {
    ...
    window.abrirAjusteVenda = function(saleKey) {
        ...
        // Reconstrói o resumo de pagamento (somente leitura) se a venda já estiver paga - usa a lista
        // completa de formas de pagamento (venda.pagamentos) quando existir, ou monta uma lista de um
        // item só a partir dos campos antigos (contaPagamentoKey/Nome) para vendas salvas antes desse ajuste.
        if (venda.status === 'PAGO') {
            let pagamentos = venda.pagamentos || [];
            if (pagamentos.length === 0 && venda.contaPagamentoNome && (venda.valorPago || 0) > 0) {
                pagamentos = [{ contaKey: venda.contaPagamentoKey || null, contaNome: venda.contaPagamentoNome, valor: parseFloat(venda.valorPago || 0) }];
            }
            pendingPagamento = {
                valorPago: parseFloat(venda.valorPago || 0), creditoUsado: parseFloat(venda.creditoUsado || 0),
                creditoGerado: parseFloat(venda.creditoGerado || 0), clienteKey: null, pagamentos
            };
        } else {
            pendingPagamento = null;
        }
    ...
    function initDatabaseSync() {
        ...
        onValue(ref(db, 'prazosPagamento'), (snap) => {
            allLoadedPrazosPagamento = snap.val() || {};

            const tbody = document.getElementById('prazos-table-body');
            tbody.innerHTML = '';
            Object.keys(allLoadedPrazosPagamento).forEach(k => {
                const prazo = allLoadedPrazosPagamento[k];
                const tr = document.createElement('tr');
                tr.innerHTML = `<td><strong>${prazo.nome}</strong></td><td>${(prazo.dias || []).join('/')} dias</td><td><button class="btn-action-prod btn-delete-prod" onclick="removePrazoPagamento('${k}')">✕</button></td>`;
                tbody.appendChild(tr);
            });

            renderSelectPrazosPagamento();
        });
        ...
        function renderCatalogoProdutos() {
            ...
            // Remove da seleção produtos que não existem mais (ex: excluídos por outra aba/usuário)
            selecionadosCatalogo.forEach(key => { if (!data[key]) selecionadosCatalogo.delete(key); });
            ...
            let lista = Object.keys(data).map(key => {
                ...
                const ultimoCusto = getCustoEfetivoProduto(prod, key);
            ...
            if (lista.length === 0) {
                tbodyTable.innerHTML = '<tr><td colspan="8" style="text-align:center; color:var(--text-faint); padding:20px;">Nenhum produto encontrado.</td></tr>';
                atualizarBotaoPrecosMassaCatalogo();
                return;
            }
            ...
            listaOrdenada.forEach(item => {
                const tr = document.createElement('tr');
                tr.innerHTML = `
                    <td><input type="checkbox" class="chk-catalogo-produto" data-key="${item.key}" ${selecionadosCatalogo.has(item.key) ? 'checked' : ''}></td>
                ...
                tr.querySelector('.chk-catalogo-produto').addEventListener('change', (e) => {
                    if (e.target.checked) selecionadosCatalogo.add(item.key);
                    else selecionadosCatalogo.delete(item.key);
                    atualizarBotaoPrecosMassaCatalogo();
                });
        ...
        function renderSuprimentosTable() {
            ...
            if (filtroBuscaSuprimentos.trim() !== '') {
                ...
                lista = lista.filter(item => normalizarBusca(item.name).includes(termo) || normalizarBusca(item.barcode).includes(termo) || String(item.codigoInterno).padStart(3, '0').includes(termo.trim()));
            }
        ...
        function renderAnaliseMaisVendidos() {
            ...
            let lista = Object.values(somaPorProduto).map(item => {
                ...
                const custo = prod ? getCustoEfetivoProduto(prod, item.key) : 0;
        ...
        function calcularDREPorMes() {
            ...
            Object.values(allLoadedSales).forEach(s => {
                ...
                s.items.forEach(item => {
                    ...
                    if (item.productKey && allLoadedProducts[item.productKey]) {
                        dados.cmv += getCustoEfetivoProduto(allLoadedProducts[item.productKey], item.productKey) * qtd;
                    }
                });
            });
        ...
        function renderAnaliseContasReceber() {
            ...
            Object.values(allLoadedSales).forEach(s => {
                if (s.origemCredito) return;
                if (s.status !== 'CONFIRMADA') return;
                const { chave: clienteKey, nome: cliente } = getClienteAgrupamento(s);
                ...
                if (!porCliente[clienteKey]) porCliente[clienteKey] = { cliente, total: 0, qtd: 0, vencimentoMaisAntigo: null, temVencida: false, temSemVencimento: false };
                porCliente[clienteKey].total += valor;
                porCliente[clienteKey].qtd += 1;
                if (temVencimento && (!porCliente[clienteKey].vencimentoMaisAntigo || vencimento < porCliente[clienteKey].vencimentoMaisAntigo)) {
                    porCliente[clienteKey].vencimentoMaisAntigo = vencimento;
                }
                if (vencida) porCliente[clienteKey].temVencida = true;
                if (!temVencimento) porCliente[clienteKey].temSemVencimento = true;
        ...
        function renderAnaliseRankingClientes() {
            ...
            Object.values(allLoadedSales).forEach(s => {
                ...
                const { chave: clienteKey, nome: cliente } = getClienteAgrupamento(s);
                if (!porCliente[clienteKey]) porCliente[clienteKey] = { cliente, totalComprado: 0, qtdCompras: 0, ultimaCompra: null };
                porCliente[clienteKey].totalComprado += (s.totalValue || 0);
                porCliente[clienteKey].qtdCompras += 1;
                if (!porCliente[clienteKey].ultimaCompra || s.salesDate > porCliente[clienteKey].ultimaCompra) {
                    porCliente[clienteKey].ultimaCompra = s.salesDate;
                }
            });
        ...
        function renderAnaliseFornecedores() {
            ...
            Object.values(allLoadedComprasSuprimentos).forEach(c => {
                ...
                // CORREÇÃO DE BUG: agrupava pelo texto de c.fornecedorNome, então o mesmo fornecedor
                // cadastrado com nome digitado de forma levemente diferente entre compras (maiúscula,
                // espaço) virava duas linhas no ranking. Toda compra já grava fornecedorKey (a chave
                // única do cadastro - Aba 9), então agrupa por ela; só cai pro nome em compras muito
                // antigas que não tenham essa chave.
                const chaveFornecedor = c.fornecedorKey || ('n:' + (c.fornecedorNome || 'Fornecedor não informado').trim().toLowerCase());
                const fornecedor = (c.fornecedorKey && allLoadedFornecedores[c.fornecedorKey]) ? allLoadedFornecedores[c.fornecedorKey].name : (c.fornecedorNome || 'Fornecedor não informado');
                const valor = parseFloat(c.valorTotal || 0);
                if (!porFornecedor[chaveFornecedor]) porFornecedor[chaveFornecedor] = { fornecedor, total: 0, qtd: 0, ultimaCompra: null };
                porFornecedor[chaveFornecedor].total += valor;
                porFornecedor[chaveFornecedor].qtd += 1;
                if (!porFornecedor[chaveFornecedor].ultimaCompra || (c.data && c.data > porFornecedor[chaveFornecedor].ultimaCompra)) {
                    porFornecedor[chaveFornecedor].ultimaCompra = c.data;
                }
                totalGeral += valor;
                qtdGeral += 1;
            });
        ...
        function renderAnaliseProducao() {
            ...
            Object.values(allLoadedProductions).forEach(op => {
                ...
                // CORREÇÃO DE BUG: agrupava pelo texto de op.name, então uma OP renomeada ou com nome
                // digitado de forma levemente diferente virava um produto separado na tabela. Usa a
                // chave única do produto (productKey, gravada em cada OP - ver saveProductionOrder); só
                // cai pro nome em OPs antigas sem essa chave.
                const chaveProduto = op.productKey || ('n:' + (op.name || 'Sem nome').trim().toLowerCase());
                const nomeProduto = (op.productKey && allLoadedProducts[op.productKey]) ? allLoadedProducts[op.productKey].name : (op.name || 'Sem nome');
                if (!porProduto[chaveProduto]) porProduto[chaveProduto] = { produto: nomeProduto, qtdOrdens: 0, qtdProduzida: 0, custoTotal: 0 };
                porProduto[chaveProduto].qtdOrdens += 1;
                porProduto[chaveProduto].qtdProduzida += yieldQty;
                porProduto[chaveProduto].custoTotal += totalCost;
            });
        ...
        function renderAnaliseDescontos() {
            ...
            Object.values(allLoadedSales).forEach(s => {
                ...
                // CORREÇÃO DE BUG: as duas tabelas agrupavam por texto (nome do produto/nome do cliente),
                // então variações de digitação ou renomeações viravam linhas separadas. Produto agrupa
                // pela chave única (productKey, já disponível em todo item de venda); cliente agrupa pela
                // chave única do cadastro (clientKey - ver getClienteAgrupamento).
                const { chave: clienteKey, nome: cliente } = getClienteAgrupamento(s);
                s.items.forEach(item => {
                    if (!item.productKey) return;
                    ...
                    const nomeProdutoAtual = allLoadedProducts[item.productKey] ? allLoadedProducts[item.productKey].name : (item.productName || 'Produto');
                    if (!porProduto[item.productKey]) porProduto[item.productKey] = { produto: nomeProdutoAtual, qtdItens: 0, total: 0 };
                    porProduto[item.productKey].qtdItens += 1;
                    porProduto[item.productKey].total += descontoItem;

                    if (!porCliente[clienteKey]) porCliente[clienteKey] = { cliente, total: 0 };
                    porCliente[clienteKey].total += descontoItem;
                });
            });
        ...
        function renderAnaliseRentabilidadeCliente() {
            ...
            Object.values(allLoadedSales).forEach(s => {
                ...
                s.items.forEach(item => {
                    if (!item.productKey) return;
                    const prod = allLoadedProducts[item.productKey];
                    const custoUnitario = prod ? getCustoEfetivoProduto(prod, item.productKey) : 0;
                    ...
                    if (!porCliente[clienteKey]) porCliente[clienteKey] = { cliente, faturamento: 0, custo: 0 };
                    porCliente[clienteKey].faturamento += faturamentoItem;
                    porCliente[clienteKey].custo += custoItem;
                });
            });
    ...
    // BUSCA DE PRODUTO NO CAMPO "NOME DO PRODUTO" DA ORDEM DE PRODUÇÃO (ABA 1)
    const recipeNameSearchInput = document.getElementById('recipe-name-search');
    const recipeNameDropdown = document.getElementById('recipe-name-dropdown');
    recipeNameSearchInput.addEventListener('input', () => {
        // Se o usuário apagar/alterar o texto, o valor selecionado antigo deixa de valer até
        // que ele escolha um item da lista de novo.
        document.getElementById('recipe-name').value = '';
        renderRecipeNameDropdown(recipeNameSearchInput.value);
    });
    recipeNameSearchInput.addEventListener('focus', () => renderRecipeNameDropdown(recipeNameSearchInput.value));
    recipeNameSearchInput.addEventListener('blur', () => setTimeout(() => recipeNameDropdown.classList.remove('open'), 150));
    ...
    // ===== ABA 7 - PEDIDOS DE COMPRA (guarda também, por dentro de cada pedido, o array `recebimentos` =====
    // ===== - a conferência física de mercadoria sem nota fiscal, separada da parte fiscal/financeira) =====
    onValue(ref(db, 'pedidosCompra'), (snapshot) => {
        allLoadedPedidosCompra = snapshot.val() || {};
        renderPedidosCompraTable();
        renderRecebimentosTable();
        // Se o modal de detalhe de um pedido estiver aberto no momento em que essa mudança chegar
        // (ex.: acabou de salvar um Recebimento ou uma Nota Fiscal), recarrega o conteúdo dele com os
        // dados novos, em vez de deixar o usuário olhando pra números desatualizados.
        if (pedidoDetalheAbertoKey && allLoadedPedidosCompra[pedidoDetalheAbertoKey] && document.getElementById('modal-detalhe-pedido-compra').style.display === 'flex') {
            abrirDetalhePedidoCompra(pedidoDetalheAbertoKey);
        }
    });
    ...
    function renderComprasTable() {
        ...
        if (lista.length === 0) {
            tbody.innerHTML = '<tr><td colspan="7" style="text-align:center; color:var(--text-faint); padding:20px;">Nenhuma compra registrada ainda.</td></tr>';
            return;
        }
        ...
        lista.forEach(c => {
            ...
            const vencida = statusPagamento === 'PENDENTE' && c.dataVencimento && c.dataVencimento < hoje;
            let vencimentoDisplay;
            let statusDisplay;
            if (statusPagamento === 'PAGO') {
                vencimentoDisplay = '<span style="color:var(--text-faint);">-</span>';
                statusDisplay = `<span class="badge-status status-pago">Pago</span>`;
            } else {
                vencimentoDisplay = c.dataVencimento
                    ? `<span style="color:${vencida ? 'var(--color-negative)' : 'var(--text-strong)'};">${dataISOparaBR(c.dataVencimento)}</span>`
                    : '<span style="color:var(--text-faint);">-</span>';
                statusDisplay = `<span class="badge-status ${vencida ? 'status-cancelado' : 'status-confirmado'}">${vencida ? '⚠️ Vencida' : 'Pendente'}</span>`;
            }
    ...
    // ===== RECEBIMENTOS DE MERCADORIA (visão logística: o que já chegou fisicamente, vindo de DUAS =====
    // ===== origens diferentes - comprasSuprimentos (nota fiscal, já com financeiro) e o array =====
    // ===== `recebimentos` dentro de cada Pedido de Compra (só conferência física, sem nota) =====
    function renderRecebimentosTable() {
        ...
        let listaNotas = Object.keys(allLoadedComprasSuprimentos)
            .map(key => ({ key, ...allLoadedComprasSuprimentos[key], tipoLinha: 'nota' }))
            .filter(c => !filtroVinculo || (filtroVinculo === 'vinculado' ? !!c.pedidoCompraKey : !c.pedidoCompraKey));

        // Recebimento sem nota sempre está vinculado a um Pedido (não existe recebimento avulso) -
        // por isso só entra na lista quando o filtro não pede especificamente "só avulsos".
        let listaRecebimentos = [];
        if (filtroVinculo !== 'avulso') {
            Object.keys(allLoadedPedidosCompra).forEach(pedidoKey => {
                const pedido = allLoadedPedidosCompra[pedidoKey];
                (pedido.recebimentos || []).forEach((r, idx) => {
                    listaRecebimentos.push({
                        key: `${pedidoKey}__${idx}`, pedidoCompraKey: pedidoKey, tipoLinha: 'recebimento',
                        data: r.data, fornecedorNome: pedido.fornecedorNome, itens: r.itens, timestamp: r.timestamp
                    });
                });
            });
        }

        let lista = [...listaNotas, ...listaRecebimentos];
        lista.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
        lista = ordenarPorEstado(lista.map(c => ({ ...c, fornecedor: c.fornecedorNome || c.fornecedor || '', valorTotal: c.tipoLinha === 'nota' ? parseFloat(c.valorTotal || 0) : -1 })), 'recebimentos-table');

        if (lista.length === 0) {
            tbody.innerHTML = '<tr><td colspan="7" style="text-align:center; color:var(--text-faint); padding:20px;">Nenhum recebimento de mercadoria registrado ainda.</td></tr>';
            return;
        }
        ...
        lista.forEach(c => {
            ...
            const tipoBadge = c.tipoLinha === 'nota'
                ? `<span class="badge-status" style="background-color:var(--color-accent-orange); color:#fff;">🧾 Com Nota Fiscal</span>`
                : `<span class="badge-status status-confirmado">📦 Só Recebimento</span>`;
            const valorDisplay = c.tipoLinha === 'nota' ? `R$ ${formatMoeda(parseFloat(c.valorTotal || 0))}` : '<span style="color:var(--text-faint);">— (sem nota ainda)</span>';
            const acaoBtn = c.tipoLinha === 'nota'
                ? `<button class="btn-action-prod btn-edit-prod" onclick="abrirDetalheCompra('${c.key}')">👁️ Ver</button>`
                : (pedidoVinculado ? `<button class="btn-action-prod btn-edit-prod" onclick="abrirDetalhePedidoCompra('${c.pedidoCompraKey}')">👁️ Ver Pedido</button>` : '');
            const tr = document.createElement('tr');
            tr.innerHTML = `
                <td>${dataFormatada}</td>
                <td>${pedidoDisplay}</td>
                <td>${c.fornecedorNome || c.fornecedor || '<span style="color:var(--text-faint);">-</span>'}</td>
                <td><span class="produtos-badge" data-itens='${encodeURIComponent(JSON.stringify(c.itens || []))}'>${qtdTipos} ${qtdTipos === 1 ? 'item' : 'itens'}</span></td>
                <td>${tipoBadge}</td>
                <td style="font-weight:700; color:#a85209;">${valorDisplay}</td>
                <td>${acaoBtn}</td>
            `;
            tbody.appendChild(tr);
        });
    ...
    // no selo "produtos" do Histórico de Vendas (Aba 5). Os itens vêm prontos no próprio atributo
    // data-itens da linha (funciona tanto pra notas fiscais quanto pra recebimentos sem nota).
    (function initItensRecebimentoTooltip() {
        ...
        function montarTabelaItensRecebidos(itensCodificados) {
            let itens = [];
            try { itens = JSON.parse(decodeURIComponent(itensCodificados || '')) || []; } catch (e) { itens = []; }
            const linhas = itens.map(it => `<tr><td>${nomeClicavelInsumo(it.insumoKey, it.insumoNome || it.descricao || 'Item')}</td><td class="tt-qtd">${formatQuantidade(it.quantidade || 0)}</td></tr>`).join('');
        ...
        tbodyReceb.addEventListener('mouseover', (e) => {
            const badge = e.target.closest('.produtos-badge');
            if (!badge) return;
            tooltip.innerHTML = montarTabelaItensRecebidos(badge.dataset.itens);
            const rect = badge.getBoundingClientRect();
            const margem = 10;
            const ttWidth = tooltip.offsetWidth;
            const ttHeight = tooltip.offsetHeight;
    ...
    function abrirDetalheCompra(key) {
        ...
        const statusPagamento = compra.statusPagamento || 'PAGO'; // compras antigas (antes dessa funcionalidade) já eram sempre pagas na hora
        const hoje = formatDateInputValue(new Date());
        const vencida = statusPagamento === 'PENDENTE' && compra.dataVencimento && compra.dataVencimento < hoje;
        const statusEl = document.getElementById('detalhe-compra-status-pagamento');
        const vencimentoWrap = document.getElementById('detalhe-compra-vencimento-wrap');
        if (statusPagamento === 'PAGO') {
            statusEl.innerHTML = `<span class="badge-status status-pago">Pago</span>`;
            vencimentoWrap.style.display = 'none';
        } else {
            statusEl.innerHTML = `<span class="badge-status ${vencida ? 'status-cancelado' : 'status-confirmado'}">${vencida ? '⚠️ Vencida' : 'Pendente'}</span>`;
            if (compra.dataVencimento) {
                vencimentoWrap.style.display = 'block';
                document.getElementById('detalhe-compra-vencimento').innerHTML = `<span style="color:${vencida ? 'var(--color-negative)' : 'var(--text-strong)'};">${dataISOparaBR(compra.dataVencimento)}</span>`;
            } else {
                vencimentoWrap.style.display = 'none';
            }
        }

        const btnDarBaixa = document.getElementById('detalhe-compra-btn-dar-baixa');
        if (statusPagamento === 'PENDENTE') {
            btnDarBaixa.style.display = 'inline-block';
            btnDarBaixa.onclick = () => {
                document.getElementById('modal-detalhe-compra').style.display = 'none';
                abrirModalDarBaixaCompra(key);
            };
        } else {
            btnDarBaixa.style.display = 'none';
            btnDarBaixa.onclick = null;
        }

        const btnExcluir = document.getElementById('detalhe-compra-btn-excluir');
        btnExcluir.onclick = () => {
            document.getElementById('modal-detalhe-compra').style.display = 'none';
            excluirCompra(key);
        };
    ...
    function resetFormNovaCompra() {
        ...
        document.getElementById('nova-compra-titulo').innerText = 'Nova Compra de Suprimentos / Matérias-Primas';
    ...
    document.getElementById('form-nova-compra').addEventListener('submit', (e) => {
        ...
        const executarCompra = () => {
            ...
            // Se essa nota está vinculada a um Pedido de Compra, monta um mapa insumoKey -> item do
            // pedido, pra saber quanto de cada item já chegou fisicamente (via "📦 Registrar
            // Recebimento") mas ainda não foi faturado - essa parte NÃO deve entrar em estoque de novo
            // aqui, senão duplicaria o que o recebimento físico já lançou.
            const pedidoVinculado = pedidoCompraKeyVinculado ? allLoadedPedidosCompra[pedidoCompraKeyVinculado] : null;
            const pedidoItemPorInsumo = {};
            if (pedidoVinculado) {
                (pedidoVinculado.itens || []).forEach(it => { pedidoItemPorInsumo[it.insumoKey] = it; });
            }
            ...
            Object.keys(dadosPorInsumo).forEach(insumoKey => {
                ...
                // Quanto desse item já está fisicamente em estoque (recebido) mas ainda sem nota -
                // essa parte só é "faturada" agora, sem entrar em estoque de novo. O que sobrar (nota
                // maior do que o que já tinha sido recebido) é estoque genuinamente novo.
                const itPedido = pedidoItemPorInsumo[insumoKey];
                const jaRecebidoNaoFaturado = itPedido ? Math.max(0, parseFloat(itPedido.qtdRecebida || 0) - parseFloat(itPedido.qtdFaturada || 0)) : 0;
                info.qtdEstoqueAdicionar = parseFloat(Math.max(0, info.somaQuantidade - jaRecebidoNaoFaturado).toFixed(4));

                const novoEstoque = parseFloat((parseFloat(insumoAtual.quantity || 0) + info.qtdEstoqueAdicionar).toFixed(4));
            ...
            // soma o que foi FATURADO agora em cada item (casando pelo insumo), e só soma no RECEBIDO
            // a parte que ainda não tinha chegado via "📦 Registrar Recebimento" (o resto já estava
            // contado ali, e a entrada em estoque logo acima já respeitou isso do mesmo jeito).
            if (pedidoVinculado) {
                const itensPedidoAtualizados = (pedidoVinculado.itens || []).map(itPedido => {
                    const faturadoAgora = itens
                        .filter(it => it.insumoKey === itPedido.insumoKey)
                        .reduce((acc, it) => acc + parseFloat(it.quantidade || 0), 0);
                    if (faturadoAgora <= 0) return itPedido;
                    const info = dadosPorInsumo[itPedido.insumoKey];
                    const recebidoNovoAgora = info ? info.qtdEstoqueAdicionar : 0;
                    return {
                        ...itPedido,
                        qtdFaturada: parseFloat((parseFloat(itPedido.qtdFaturada || 0) + faturadoAgora).toFixed(4)),
                        qtdRecebida: parseFloat((parseFloat(itPedido.qtdRecebida || 0) + recebidoNovoAgora).toFixed(4))
                    };
                });
                ...
                const novoStatusPedido = totalmenteRecebido ? 'RECEBIDO_TOTAL' : (algumRecebido ? 'RECEBIDO_PARCIAL' : pedidoVinculado.status);
                const comprasVinculadas = [...(pedidoVinculado.comprasVinculadas || []), compraRef.key];
            ...
            Promise.all(promessas).then(() => {
                ...
                showAlert(`Nota fiscal lançada! Estoque dos insumos atualizado. ${msgPagamento}`, 'success');
                ativarComprasSubview('notas');
            });
        };
    ...
    function ligarBuscaInsumoPedido(input, dropdown, tr, campo) {
        function renderResultados() {
            ...
            const candidatos = Object.entries(allLoadedSuprimentos)
                .filter(([k, v]) => {
                    if (campo === 'nome') return normalizarBusca(v.name).includes(q);
                    if (campo === 'codigoInterno') return String(parseInt(v.codigoInterno) || 0).padStart(3, '0').includes(input.value.trim());
                    return normalizarBusca(v.barcode || '').includes(q);
                })
            ...
            candidatos.forEach(([key, insumo]) => {
                const item = document.createElement('div');
                item.className = 'client-autocomplete-item';
                item.innerHTML = `<strong>${insumo.name}</strong> <span style="color:var(--text-muted); font-size:8.5pt;">(Cód. ${String(parseInt(insumo.codigoInterno) || 0).padStart(3,'0')}${insumo.barcode ? ` · ${insumo.barcode}` : ''})</span>`;
                item.addEventListener('mousedown', (e) => {
                    e.preventDefault();
        ...
        input.addEventListener('blur', () => {
            setTimeout(() => {
                dropdown.classList.remove('open');
                // Se depois de sair do campo a linha continuar sem nenhum insumo do sistema associado
                // (usuário digitou algo que não bateu com nada), pinta os campos de vermelho pra deixar
                // claro que essa informação não existe no nosso cadastro de matérias-primas.
                atualizarEstadoNaoEncontradoLinhaPedido(tr);
            }, 150);
        });
    }

    // Marca em vermelho (bem básico, sem popup) os campos de busca de um item do pedido quando o
    // usuário digitou algo que não corresponde a nenhuma matéria-prima cadastrada no sistema - assim
    // que um insumo válido é selecionado (tr.dataset.insumoKey preenchido), a marcação some.
    function atualizarEstadoNaoEncontradoLinhaPedido(tr) {
        const campos = ['.item-pedido-codinterno', '.item-pedido-descricao', '.item-pedido-codbarras'].map(sel => tr.querySelector(sel));
        const algumCampoPreenchido = campos.some(el => el.value.trim() !== '');
        const naoEncontrado = algumCampoPreenchido && !tr.dataset.insumoKey;
        campos.forEach(el => el.classList.toggle('pedido-item-nao-encontrado', naoEncontrado));
    }

    // Preenche a linha inteira (código interno, descrição, código de barras, valor unitário) a partir
    // de um insumo selecionado em qualquer um dos três campos de busca, e marca a linha como
    // "resolvida" via data-insumo-key - só linhas com esse atributo preenchido entram no pedido ao salvar.
    function aplicarInsumoNaLinhaPedido(tr, insumoKey) {
        const insumo = allLoadedSuprimentos[insumoKey];
        if (!insumo) return;
        tr.dataset.insumoKey = insumoKey;
        tr.querySelector('.item-pedido-codinterno').value = String(parseInt(insumo.codigoInterno) || 0).padStart(3, '0');
    ...
    function criarLinhaItemPedidoCompra() {
        ...
        const tdCodInterno = document.createElement('td');
        tdCodInterno.innerHTML = `<div class="client-autocomplete-wrapper">
            <input type="text" class="item-pedido-codinterno" placeholder="Cód." autocomplete="off" style="margin:0;">
        ...
        ligarBuscaInsumoPedido(tr.querySelector('.item-pedido-codinterno'), tdCodInterno.querySelector('.client-autocomplete-dropdown'), tr, 'codigoInterno');
    ...
    function recalcularTotaisPedidoCompra() {
        ...
        const frete = Math.max(0, parseFloat(document.getElementById('pedido-compra-frete').value) || 0);
        ...
        const liquido = parseFloat((bruto - descontoValor + frete).toFixed(2));
        document.getElementById('pedido-compra-total-liquido').innerText = `R$ ${formatMoeda(liquido)}`;
        return { bruto, descontoPercentual, descontoValor, frete, liquido };
    }
    ...
    // O frete só é somado ao total do pedido - nunca é rateado entre os itens, então o valor unitário
    // de cada produto continua refletindo só o preço dele mesmo, sem embutir o custo do frete.
    document.getElementById('pedido-compra-frete').addEventListener('input', recalcularTotaisPedidoCompra);
    ...
    document.getElementById('btn-trigger-modal-novo-pedido-compra').addEventListener('click', () => {
        ...
        document.getElementById('pedido-compra-frete').value = 0;
        recalcularTotaisPedidoCompra();
    });
    ...
    document.getElementById('form-novo-pedido-compra').addEventListener('submit', (e) => {
        ...
        if (itemInvalido || itens.length === 0) { showAlert('Preencha corretamente todos os itens do pedido: busque e selecione o produto (por código interno, nome ou código de barras) e informe a quantidade desejada.', 'warning'); return; }
        ...
        const { bruto, descontoPercentual, descontoValor, frete, liquido } = recalcularTotaisPedidoCompra();

        // Guarda uma "foto" do prazo escolhido (nome + dias) além da chave, pra continuar mostrando
        // certo no pedido mesmo que aquele prazo seja renomeado ou apagado depois em "Gerenciar Prazos".
        const prazoPagamentoKey = document.getElementById('pedido-compra-prazo-pagamento').value || null;
        const prazoSelecionado = prazoPagamentoKey ? allLoadedPrazosPagamento[prazoPagamentoKey] : null;
        const prazoPagamentoNome = prazoSelecionado ? prazoSelecionado.nome : null;
        const prazoPagamentoDias = prazoSelecionado ? prazoSelecionado.dias : null;
        ...
        if (pedidoCompraEditandoKey) {
            ...
            const pedidoAtualizado = {
                ...
                prazoPagamentoKey, prazoPagamentoNome, prazoPagamentoDias,
                valorBruto: bruto, descontoPercentual, descontoValor, valorFrete: frete, valorLiquido: liquido
            };
    ...
    document.getElementById('close-modal-detalhe-pedido-compra').addEventListener('click', () => { pedidoDetalheAbertoKey = null; });
    window.abrirDetalhePedidoCompra = function(key) {
        const p = allLoadedPedidosCompra[key];
        if (!p) return;
        pedidoDetalheAbertoKey = key;
        const statusInfo = STATUS_PEDIDO_LABELS[p.status] || STATUS_PEDIDO_LABELS.ENVIADO;
        ...
        document.getElementById('detalhe-pedido-previsao').innerHTML = p.dataVencimento
            ? `<span style="${vencida ? 'color:var(--color-negative);' : ''}">${vencida ? '⚠️ ' : ''}${dataISOparaBR(p.dataVencimento)}</span>${p.prazoPagamentoNome ? ` <span style="font-size:8pt; color:var(--text-muted); font-weight:400;">(Prazo: ${p.prazoPagamentoNome})</span>` : ''}`
            : '-';
        ...
        // Recebimento físico (estoque) e faturamento fiscal/financeiro agora são independentes: um
        // pedido pode estar com tudo recebido mas ainda faltando nota, ou já com nota lançada pra uma
        // parte que ainda nem chegou fisicamente - por isso cada botão usa sua própria pendência.
        const itensPedido = p.itens || [];
        const restanteFisico = itensPedido.reduce((acc, it) => acc + Math.max(0, parseFloat(it.qtdPedida || 0) - parseFloat(it.qtdRecebida || 0)), 0);
        const restanteFiscal = itensPedido.reduce((acc, it) => acc + Math.max(0, parseFloat(it.qtdPedida || 0) - parseFloat(it.qtdFaturada || 0)), 0);
        const podeReceberFisico = p.status !== 'CANCELADO' && restanteFisico > 0.0001;
        const podeReceberFiscal = p.status !== 'CANCELADO' && restanteFiscal > 0.0001;
        const podeCancelar = p.status !== 'RECEBIDO_TOTAL' && p.status !== 'CANCELADO';
        const podeExcluir = (p.comprasVinculadas || []).length === 0 && (p.recebimentos || []).length === 0;
        document.getElementById('detalhe-pedido-acoes').innerHTML = `
            ...
            ${podeReceberFisico ? `<button type="button" class="btn-pedido-acao-principal btn-pedido-acao-recebimento" onclick="abrirModalRegistrarRecebimento('${key}')">📦 Registrar Recebimento</button>` : ''}
            ${podeReceberFiscal ? `<button type="button" class="btn-pedido-acao-principal btn-pedido-acao-nota-fiscal" onclick="receberNotaFiscalDePedido('${key}')">🧾 Lançar Nota Fiscal</button>` : ''}
            ${podeCancelar ? `<button type="button" class="btn-action-prod btn-delete-prod" onclick="cancelarPedidoCompra('${key}')">✕ Cancelar</button>` : ''}
        ...
        if (p.valorLiquido !== undefined) {
            ...
            document.getElementById('detalhe-pedido-frete').innerText = `R$ ${formatMoeda(parseFloat(p.valorFrete || 0))}`;
        ...
        tbody.innerHTML = itensPedido.map(it => {
            const completoRecebido = parseFloat(it.qtdRecebida || 0) >= parseFloat(it.qtdPedida || 0) - 0.0001;
            const corRecebida = completoRecebido ? 'var(--color-positive)' : (parseFloat(it.qtdRecebida || 0) > 0 ? '#a85209' : 'var(--text-muted)');
            const completoFaturado = parseFloat(it.qtdFaturada || 0) >= parseFloat(it.qtdPedida || 0) - 0.0001;
            const corFaturada = completoFaturado ? 'var(--color-positive)' : (parseFloat(it.qtdFaturada || 0) > 0 ? '#a85209' : 'var(--text-muted)');
            return `<tr>
                ...
                <td style="color:${corFaturada}; font-weight:700;">${formatQuantidade(it.qtdFaturada || 0)} ${it.unidade || ''}</td>
                <td>R$ ${formatMoeda(parseFloat(it.precoEstimado || 0))}</td>
            </tr>`;
        }).join('');

        const recebimentosWrap = document.getElementById('detalhe-pedido-recebimentos-lista');
        const recebimentos = p.recebimentos || [];
        recebimentosWrap.innerHTML = recebimentos.length === 0 ? 'Nenhum recebimento registrado ainda.' : recebimentos.map(r => {
            const qtdItens = (r.itens || []).length;
            return `<div style="padding:6px 0; border-bottom:1px solid var(--border-soft);">📦 ${dataISOparaBR(r.data)} - ${qtdItens} ${qtdItens === 1 ? 'item' : 'itens'}${r.observacoes ? ` <span style="color:var(--text-faint);">(${r.observacoes})</span>` : ''}</div>`;
        }).join('');
        ...
        notasWrap.innerHTML = notas.length === 0 ? 'Nenhuma nota fiscal lançada ainda.' : notas.map(c =>
    ...
    window.editarPedidoCompra = function(key) {
        ...
        renderSelectPrazosPagamento(p.prazoPagamentoKey || '');
        ...
        (p.itens || []).forEach(it => {
            ...
            tr.querySelector('.item-pedido-codinterno').value = insumoAtual ? String(parseInt(insumoAtual.codigoInterno) || 0).padStart(3, '0') : '';
        ...
        document.getElementById('pedido-compra-frete').value = p.valorFrete || 0;
        recalcularTotaisPedidoCompra();
    ...
    window.excluirPedidoCompra = function(key) {
        ...
        if ((p.recebimentos || []).length > 0) {
            showAlert('Não é possível excluir este pedido pois já tem recebimento(s) de mercadoria registrado(s), que já deram entrada em estoque.', 'danger');
            return;
        }
    ...
    // Preenche o modal de Nota Fiscal (modal-nova-compra) com fornecedor + itens de um Pedido de
    // Compra, na quantidade que AINDA FALTA FATURAR (qtdFaturada, não qtdRecebida - um item pode já
    // ter chegado via "📦 Registrar Recebimento" e mesmo assim ainda estar pendente de nota).
    window.receberNotaFiscalDePedido = function(key) {
        ...
        document.getElementById('nova-compra-titulo').innerText = `🧾 Lançar Nota Fiscal - Pedido ${String(p.pedidoNumero).padStart(3,'0')}`;
        ...
        document.getElementById('compra-itens-lista').innerHTML = '';
        (p.itens || []).forEach(it => {
            const restante = parseFloat((parseFloat(it.qtdPedida || 0) - parseFloat(it.qtdFaturada || 0)).toFixed(4));
            if (restante <= 0) return; // item já totalmente faturado em uma nota anterior
    ...
    // ===================================================================================
    // ===== REGISTRAR RECEBIMENTO DE MERCADORIA (só confere a chegada física de um item =====
    // ===== do Pedido de Compra - dá entrada em estoque na hora, sem CNPJ, forma de =====
    // ===== pagamento ou nota fiscal, que ficam separados no botão "🧾 Nota Fiscal") =====
    // ===================================================================================
    setupModalEvents(null, 'modal-registrar-recebimento', 'close-modal-registrar-recebimento');

    window.abrirModalRegistrarRecebimento = function(key) {
        const p = allLoadedPedidosCompra[key];
        if (!p) return;

        document.getElementById('recebimento-pedido-key').value = key;
        document.getElementById('recebimento-pedido-numero').innerText = `Pedido ${String(p.pedidoNumero).padStart(3,'0')}`;
        document.getElementById('recebimento-pedido-fornecedor').innerText = p.fornecedorNome;
        document.getElementById('recebimento-data').value = dataISOparaBR(formatDateInputValue(new Date()));
        document.getElementById('recebimento-observacoes').value = '';

        const tbody = document.getElementById('recebimento-itens-table-body');
        tbody.innerHTML = '';
        (p.itens || []).forEach(it => {
            const restante = parseFloat((parseFloat(it.qtdPedida || 0) - parseFloat(it.qtdRecebida || 0)).toFixed(4));
            if (restante <= 0) return; // item já totalmente recebido
            const tr = document.createElement('tr');
            tr.dataset.insumoKey = it.insumoKey;
            tr.dataset.unidade = it.unidade || '';
            tr.innerHTML = `
                <td>${it.insumoNome}</td>
                <td>${formatQuantidade(it.qtdPedida)} ${it.unidade || ''}</td>
                <td>${formatQuantidade(it.qtdRecebida || 0)} ${it.unidade || ''}</td>
                <td><input type="number" class="recebimento-item-qtd" min="0" max="${restante}" step="any" value="${restante}" style="margin:0; width:100%;"></td>
            `;
            tbody.appendChild(tr);
        });

        const semItens = tbody.children.length === 0;
        document.getElementById('recebimento-sem-itens-aviso').style.display = semItens ? 'block' : 'none';
        document.getElementById('btn-salvar-recebimento').style.display = semItens ? 'none' : 'block';

        document.getElementById('modal-detalhe-pedido-compra').style.display = 'none';
        document.getElementById('modal-registrar-recebimento').style.display = 'flex';
    };

    document.getElementById('form-registrar-recebimento').addEventListener('submit', (e) => {
        e.preventDefault();
        const key = document.getElementById('recebimento-pedido-key').value;
        const p = allLoadedPedidosCompra[key];
        if (!p) return;

        const dataRecebimento = dataBRparaISO(document.getElementById('recebimento-data').value);
        if (!dataRecebimento) { showAlert('Informe uma data de recebimento válida.', 'warning'); return; }

        const observacoes = document.getElementById('recebimento-observacoes').value.trim();

        const itensRecebidosAgora = [];
        let algumInvalido = false;
        document.querySelectorAll('#recebimento-itens-table-body tr').forEach(tr => {
            const insumoKey = tr.dataset.insumoKey;
            const unidade = tr.dataset.unidade;
            const input = tr.querySelector('.recebimento-item-qtd');
            const qtd = parseFloat(input.value);
            const max = parseFloat(input.max);
            if (isNaN(qtd) || qtd < 0 || qtd > max + 0.0001) { algumInvalido = true; return; }
            if (qtd > 0.0001) {
                const itPedido = (p.itens || []).find(i => i.insumoKey === insumoKey);
                itensRecebidosAgora.push({ insumoKey, insumoNome: itPedido ? itPedido.insumoNome : '', quantidade: parseFloat(qtd.toFixed(4)), unidade });
            }
        });

        if (algumInvalido) { showAlert('A quantidade recebida não pode ser maior que a quantidade restante nem negativa.', 'warning'); return; }
        if (itensRecebidosAgora.length === 0) { showAlert('Informe ao menos uma quantidade recebida maior que zero.', 'warning'); return; }

        const timestamp = Date.now();
        const promessas = [];

        // Dá entrada direta no estoque de cada insumo recebido. Sem custo/nota ainda, o preço do
        // insumo (Aba 3) não é mexido aqui - isso só acontece quando a Nota Fiscal correspondente
        // for lançada depois, com o valor de verdade.
        itensRecebidosAgora.forEach(it => {
            const insumoAtual = allLoadedSuprimentos[it.insumoKey];
            if (!insumoAtual || insumoAtual.naoEhMateriaPrima) return;
            const novoEstoque = parseFloat((parseFloat(insumoAtual.quantity || 0) + it.quantidade).toFixed(4));
            promessas.push(update(ref(db, `suprimentos/${it.insumoKey}`), { quantity: novoEstoque }));
        });

        // Atualiza a quantidade recebida (física) de cada item do pedido e recalcula o status geral -
        // qtdFaturada não é tocada aqui, fica só a cargo da Nota Fiscal.
        const itensPedidoAtualizados = (p.itens || []).map(itPedido => {
            const recebidoAgora = itensRecebidosAgora.filter(it => it.insumoKey === itPedido.insumoKey).reduce((acc, it) => acc + it.quantidade, 0);
            return { ...itPedido, qtdRecebida: parseFloat((parseFloat(itPedido.qtdRecebida || 0) + recebidoAgora).toFixed(4)) };
        });
        const totalmenteRecebido = itensPedidoAtualizados.every(it => it.qtdRecebida >= it.qtdPedida - 0.0001);
        const algumRecebido = itensPedidoAtualizados.some(it => it.qtdRecebida > 0.0001);
        const novoStatusPedido = totalmenteRecebido ? 'RECEBIDO_TOTAL' : (algumRecebido ? 'RECEBIDO_PARCIAL' : p.status);
        const novoRecebimento = { data: dataRecebimento, itens: itensRecebidosAgora, observacoes: observacoes || null, timestamp };
        const recebimentosAtualizados = [...(p.recebimentos || []), novoRecebimento];

        promessas.push(update(ref(db, `pedidosCompra/${key}`), { itens: itensPedidoAtualizados, status: novoStatusPedido, recebimentos: recebimentosAtualizados }));

        Promise.all(promessas).then(() => {
            document.getElementById('modal-registrar-recebimento').style.display = 'none';
            showAlert('Recebimento registrado! Estoque atualizado. Quando a nota fiscal chegar, use o botão "🧾 Nota Fiscal" para lançar os dados financeiros.', 'success');
            ativarComprasSubview('recebimentos');
        }).catch(err => showAlert('Erro ao registrar recebimento: ' + err.message, 'danger'));
    });
    ...
    function statusOrcamentoAtivo(status) {
        return status !== 'CANCELADO';
    }

    // Caixa disponível = saldo atual de todas as contas + total previsto a receber (vendas CONFIRMADA/fiado em aberto).
    // Mesma regra usada no card "Contas a Receber" da Aba Análise, só que replicada aqui pois aquela
    // função vive dentro do escopo do initDatabaseSync e não fica acessível fora dele.
    function calcularSaldoDisponivelCaixa() {
        const saldoContas = Object.values(allLoadedContas).reduce((acc, c) => acc + parseFloat(c.saldo || 0), 0);
        let aReceber = 0;
        Object.values(allLoadedSales).forEach(s => {
            if (s.origemCredito) return;
            if (s.status !== 'CONFIRMADA') return;
            aReceber += (s.totalValue || 0);
        });
        return saldoContas + aReceber;
    }

    function renderOrcamentosCaixa() {
        const elDisponivel = document.getElementById('orcamentos-kpi-caixa-disponivel');
        if (!elDisponivel) return;

        const caixaDisponivel = calcularSaldoDisponivelCaixa();
        const comprometido = Object.values(allLoadedOrcamentos)
            .filter(o => statusOrcamentoAtivo(o.status))
            .reduce((acc, o) => acc + parseFloat(o.valorOrcado || 0), 0);

        elDisponivel.innerText = `R$ ${formatMoeda(caixaDisponivel)}`;
        document.getElementById('orcamentos-kpi-comprometido').innerText = `R$ ${formatMoeda(comprometido)}`;

        const percentual = caixaDisponivel > 0 ? (comprometido / caixaDisponivel * 100) : (comprometido > 0 ? 100 : 0);
        const corBarra = percentual >= 100 ? 'estourado' : (percentual >= 75 ? 'atencao' : 'ok');
        document.getElementById('orcamentos-caixa-progress-fill').className = `orcamento-progress-fill ${corBarra}`;
        document.getElementById('orcamentos-caixa-progress-fill').style.width = `${Math.min(100, percentual)}%`;

        const statusEl = document.getElementById('orcamentos-caixa-status');
        const diferenca = caixaDisponivel - comprometido;
        if (comprometido > caixaDisponivel) {
            statusEl.innerHTML = `⚠️ <span style="color:var(--color-negative); font-weight:700;">Seus orçamentos ativos superam o caixa disponível em R$ ${formatMoeda(Math.abs(diferenca))}.</span>`;
        } else if (percentual >= 75) {
            statusEl.innerHTML = `<span style="color:#93450a; font-weight:600;">Atenção: ${percentual.toFixed(0)}% do caixa disponível já está comprometido. Restam R$ ${formatMoeda(diferenca)} livres.</span>`;
        } else {
            statusEl.innerHTML = `Seus orçamentos ativos cabem no caixa disponível. Restam R$ ${formatMoeda(diferenca)} livres (${(100 - percentual).toFixed(0)}%).`;
        }
    }
